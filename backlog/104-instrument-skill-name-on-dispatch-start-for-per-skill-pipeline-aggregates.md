...
`dispatch_start` events in `claude/pipeline/dispatch.py:445` record model, tier, feature, task — but not which skill initiated the sub-agent dispatch. Per-skill pipeline cost aggregates are therefore not computable today. This blocks data-driven ranking of pipeline-side extraction candidates (C8, C9) and makes post-ship ROI validation impossible for anything the pipeline dispatches.
