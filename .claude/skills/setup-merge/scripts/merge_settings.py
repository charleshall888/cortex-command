...
# Required hooks: merged unconditionally into the user's settings.json.
# Every entry here must also be wired in claude/settings.json's hooks block —
# update both in the same commit if you add or remove a wired hook.
...
# Optional hooks: prompted individually. The OPTIONAL designation is
# metadata for /setup-merge's per-hook prompt flow — it tells an
# (agent-driven or interactive) caller which hooks to ask about rather
# than merge silently. Every hook file is still symlinked unconditionally
...
def discover_symlinks(repo_root: Path) -> list[dict]:
    ...
    # 4. Hooks live in two directories by design: hooks/ (shared cortex-command
    #    hooks) and claude/hooks/ (repo-specific hooks — worktree, output
    #    filter, sync-permissions). Both are walked. A hook is any file
    #    matching cortex-* in either directory, regardless of extension.
