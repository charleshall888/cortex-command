...
Synthesizer-only: each test assembles the Step 2d synthesizer prompt with a
hand-crafted reviewer envelope and asserts on the rubric's deterministic
triggers. The former full-pipeline tests (reviewer dispatch → synthesis via
`claude -p '/cortex-core:critical-review <fixture>'`) were removed when #367
was closed wontfix: the Req 9b path gate rejects test fixtures by design, and
re-calibrating the stochastic N-of-3 baselines requires real-terminal live
runs whose cost outweighed their signal (the rubric coverage lives here).
"""
...
def test_synthesizer_trigger_2_restates() -> None:
    ...
    # record (passed, reason) per attempt.
