...
#   C) Seed a no-op marker in plugins/android-dev-extras/skills/r8-analyzer/SKILL.md
...
#   D) Create plugins/cortex-unclassified/.claude-plugin/plugin.json with a
...
#   E) Seed a no-op marker directly in plugins/cortex-core/skills/requirements/SKILL.md
...
#   F) Seed drift in claude/hooks/cortex-tool-failure-tracker.sh (top-level
...
HAND_SKILL="plugins/android-dev-extras/skills/r8-analyzer/SKILL.md"
...
# --- Subtest C: hand-maintained pass-through (android-dev-extras) ---
echo "Subtest C: seed no-op marker in $HAND_SKILL"
...
if [ "$HOOK_EXIT_C" -ne 0 ]; then
    ...
    report_pass "Subtest C: hook passed hand-maintained r8-analyzer edit (exit 0)."
fi

git restore --staged "$HAND_SKILL" 2>/dev/null || true
git checkout -- "$HAND_SKILL" 2>/dev/null || true
just build-plugin >/dev/null 2>&1 || true

# --- Subtest D: unclassified-plugin fail-closed guard ---
echo "Subtest D: create unclassified plugins/cortex-unclassified/.claude-plugin/plugin.json"
...
if [ "$HOOK_EXIT_D" -eq 0 ]; then
    report_fail "Subtest D: hook exited 0 but an unclassified plugin dir was introduced (expected non-zero)."
    echo "--- hook output ---"
    echo "$HOOK_OUTPUT_D"
    echo "-------------------"
elif ! echo "$HOOK_OUTPUT_D" | grep -qE "cortex-unclassified.*(not classified|unclassified|BUILD_OUTPUT_PLUGINS|HAND_MAINTAINED_PLUGINS)"; then
    report_fail "Subtest D: hook exit $HOOK_EXIT_D but stderr does not mention the unclassified guard."
    echo "--- hook output ---"
    echo "$HOOK_OUTPUT_D"
    echo "-------------------"
else
    report_pass "Subtest D: hook fail-closed on unclassified plugin (exit $HOOK_EXIT_D)."
fi

git restore --staged plugins/cortex-unclassified/ 2>/dev/null || true
rm -rf plugins/cortex-unclassified/

# --- Subtest E: direct hand-edit to build-output plugin tree (R9 narrowing) ---
echo "Subtest E: seed no-op marker in $INTERACTIVE_SKILL without touching top-level source"
...
if [ "$HOOK_EXIT_E" -ne 0 ]; then
    report_fail "Subtest E: hook exited $HOOK_EXIT_E (expected 0 — the hand-edit is reverted from the canonical source, not blocked)."
    echo "--- hook output ---"
    echo "$HOOK_OUTPUT_E"
    echo "-------------------"
elif ! echo "$HOOK_OUTPUT_E" | grep -q "plugins/cortex-core/skills/requirements/SKILL.md"; then
    report_fail "Subtest E: hook exit 0 but output does not name the reconciled plugins/cortex-core/skills/requirements/SKILL.md."
    echo "--- hook output ---"
    echo "$HOOK_OUTPUT_E"
    echo "-------------------"
elif git show ":$INTERACTIVE_SKILL" 2>/dev/null | grep -q "drift-test-marker"; then
    report_fail "Subtest E: the staged mirror still carries the hand-edit — it should be rebuilt from the unchanged canonical source."
else
    report_pass "Subtest E: direct build-output hand-edit reconciled away from the canonical source (exit 0)."
fi
...
# --- Subtest F: top-level claude/hooks/cortex-* drift ---
...
echo "Subtest F: seed drift in $CLAUDE_HOOK_SRC"
...
F_MIRROR="plugins/cortex-overnight/hooks/cortex-tool-failure-tracker.sh"
if [ "$HOOK_EXIT_F" -ne 0 ]; then
    report_fail "Subtest F: hook exited $HOOK_EXIT_F (expected 0 — reconciliation folds the mirror in, it does not block)."
    echo "--- hook output ---"
    echo "$HOOK_OUTPUT_F"
    echo "-------------------"
elif ! echo "$HOOK_OUTPUT_F" | grep -q "$F_MIRROR"; then
    report_fail "Subtest F: hook exit 0 but output does not name the reconciled $F_MIRROR — the Phase 2 claude/hooks/cortex- trigger has regressed."
    echo "--- hook output ---"
    echo "$HOOK_OUTPUT_F"
    echo "-------------------"
elif ! git diff --cached --name-only | grep -qx "$F_MIRROR"; then
    report_fail "Subtest F: mirror $F_MIRROR was not staged into the commit."
else
    report_pass "Subtest F: claude/hooks/cortex-* edit reconciled into the commit (exit 0)."
fi

git restore --staged "$CLAUDE_HOOK_SRC" "$F_MIRROR" 2>/dev/null || true
git checkout -- "$CLAUDE_HOOK_SRC" "$F_MIRROR" 2>/dev/null || true
rm -f "$(git rev-parse --git-dir)/cortex-reconciled"
just build-plugin >/dev/null 2>&1 || true
