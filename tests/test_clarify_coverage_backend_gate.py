"""Structural regression guard — discovery clarify §2 backend gate (#321 R7).

Pins the DOCUMENTED two-arm gate, not runtime behavior: §2 must resolve the
...
construction — a positive assertion on the real §2 plus a NEGATIVE CONTROL over
the verbatim pre-edit section, so a toothless/ungated edit fails the test rather than
...
SECTION_HEADING = "### 2. Check Existing Backlog Coverage"

# The verbatim pre-edit section — the negative control. ``_gate_present`` MUST return
# False here; that is what proves the positive assertion would fail on an
# ungated/toothless section (the gate token and skip arm are simply absent).
PRE_EDIT_SECTION = (
...
def _gate_present(section: str) -> bool:
    """True only for a real §2 gate: the backend reader AND a non-local
...
def test_clarify_coverage_section_gates_scan_on_backend() -> None:
    text = CLARIFY.read_text(encoding="utf-8")
    section = _slice_section(text, SECTION_HEADING)
    assert _gate_present(section), (
        "clarify.md §2 must resolve `cortex-read-backlog-backend` and carry a "
...
def test_negative_control_pre_edit_section_is_ungated() -> None:
    # Discriminator: the verbatim pre-edit section must NOT pass — confirming the
