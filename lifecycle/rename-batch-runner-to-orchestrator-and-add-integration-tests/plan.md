...
- **Context**:
  ...
  - Only imports needed: `argparse`, `asyncio`, `from claude.overnight.orchestrator import BatchConfig, run_batch`
...
- **Context**:
  ...
  - TYPE_CHECKING block: `from claude.overnight.batch_runner import BatchResult, BatchConfig` → `from claude.overnight.orchestrator import BatchResult, BatchConfig`
...
- **Context**:
  ...
  - TYPE_CHECKING block: `from claude.overnight.batch_runner import BatchConfig` → `from claude.overnight.orchestrator import BatchConfig`
...
- **Complexity**: simple
- **Context**:
  - `__init__.py`: `from claude.overnight.batch_runner import BatchConfig, BatchResult, run_batch` → `from claude.overnight.orchestrator import BatchConfig, BatchResult, run_batch`
  - `smoke_test.py`: similar top-level import update (line numbers may vary; scan for `batch_runner`)
  - `state.py` TYPE_CHECKING block (line ~12): `from claude.overnight.batch_runner import BatchResult` → `from claude.overnight.orchestrator import BatchResult`
  - `conflict.py` TYPE_CHECKING block: `from claude.overnight.batch_runner import BatchConfig` → `from claude.overnight.orchestrator import BatchConfig`
- **Verification**: `grep -rn "from claude.overnight.batch_runner import" claude/overnight/__init__.py claude/overnight/smoke_test.py claude/overnight/state.py claude/pipeline/conflict.py` returns 0 matches
...
- **What**: Update `from claude.overnight.batch_runner import ...` in each test file to use canonical module locations. Do not change test logic — only import lines.
- **Depends on**: [2]
- **Complexity**: simple
- **Context**:
  - `test_exit_report.py` line 18: `from claude.overnight.batch_runner import BatchConfig, execute_feature` → split: `BatchConfig` from `claude.overnight.orchestrator`; `execute_feature` from `claude.overnight.feature_executor`
  - `test_trivial_conflict.py` line 32: `from claude.overnight.batch_runner import execute_feature` → `from claude.overnight.feature_executor import execute_feature`
  - `test_repair_agent.py` line 23: `from claude.overnight.batch_runner import execute_feature` → `from claude.overnight.feature_executor import execute_feature`
  - `test_overnight_state.py` line 16: `from claude.overnight.batch_runner import BatchResult` → `from claude.overnight.orchestrator import BatchResult`
- **Verification**: `grep -rn "from claude.overnight.batch_runner import" claude/overnight/tests/test_exit_report.py claude/overnight/tests/test_overnight_state.py claude/pipeline/tests/test_trivial_conflict.py claude/pipeline/tests/test_repair_agent.py` returns 0 matches
...
- **Context**:
  ...
  - Add: `from claude.overnight.types import CircuitBreakerState`
...
- **Complexity**: complex
- **Context**:
  - Line 21: `import claude.overnight.batch_runner as batch_runner_module` → `import claude.overnight.orchestrator as orchestrator_module`
  - Line 24: `from claude.overnight.batch_runner import (BatchConfig, BatchResult, CIRCUIT_BREAKER_THRESHOLD, ...)` → split imports: `BatchConfig, BatchResult` from `claude.overnight.orchestrator`; `CIRCUIT_BREAKER_THRESHOLD` from `claude.overnight.constants`; `FeatureResult` from `claude.overnight.types`; `execute_feature` from `claude.overnight.feature_executor`
  ...
  - `transition` patch target: **`claude.overnight.orchestrator.transition`** (string form: `"claude.overnight.orchestrator.transition"`) — NOT `claude.overnight.state.transition`. When `orchestrator.py` does `from claude.overnight.state import transition`, the name `transition` is bound in `orchestrator.__dict__`. Calls inside `orchestrator.py` look up `transition` in `orchestrator.__dict__`, so patching `claude.overnight.state.transition` is invisible to those calls; the correct target is `claude.overnight.orchestrator.transition`.
  ...
  - Inline local imports at lines 355, 460, 1560, 1587, 1611, 1664, 1700, 1736, 1775, 1811: `from claude.overnight.batch_runner import run_batch` → `from claude.overnight.orchestrator import run_batch`
  - Line 1467: `from claude.overnight.batch_runner import FeatureResult as _FR` → `from claude.overnight.types import FeatureResult as _FR` (destination is `claude.overnight.types`, not `orchestrator`)
...
grep -rn "from claude.overnight.batch_runner import" claude/            # 0 matches
python3 -m claude.overnight.batch_runner --help                         # exit 0
python3 -c "from claude.overnight import BatchConfig, run_batch"        # exit 0
...
- **`transition` patch target**: `claude.overnight.orchestrator.transition`, not `claude.overnight.state.transition`. When `orchestrator.py` does `from claude.overnight.state import transition`, the name `transition` is bound in `orchestrator.__dict__`. Calls inside `orchestrator.py` look up `transition` in `orchestrator.__dict__`, so patching `claude.overnight.state.transition` is invisible to those calls. The correct patch target is `claude.overnight.orchestrator.transition`.
