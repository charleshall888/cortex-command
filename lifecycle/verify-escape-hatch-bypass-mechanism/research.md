...
- RESOLVED: The `f65` dangerous shell prefix classifier does NOT block interpreter-wrapped commands in the glob-based permission evaluation path. Empirical testing confirmed that `bash -c "git push --force --dry-run origin main"`, `bash -c "echo hello"` (control), `python3 -c "import os; os.system('git push --force --dry-run origin main')"`, and `sh -c "rm -rf /tmp/test"` were all silently ALLOWED. Glob matching runs first and `Bash(bash *)` allow matches the full command string — the f65 classifier either operates only in auto-mode/compound-command contexts, or is overridden by explicit allow rules in user settings. Neither deny rules nor the classifier intervened.
- RESOLVED: Encoding variations were not tested in this spike (deferred per scope decision). Given that the baseline bypass is confirmed with standard quoting, encoding variations would only expand the attack surface further — they cannot close it. The finding that deny rules do not compose through interpreter wrappers is sufficient to justify DR-2 at high priority. Encoding variation testing is unnecessary for the 057 priority decision.
