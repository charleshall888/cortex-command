...
The DR-5 framing in epic research describes `cortex setup` as the canonical `~/.claude/` deployment subcommand. **That subcommand does not exist.** Ticket 117's spec.md (`lifecycle/build-cortex-setup-subcommand-and-retire-shareable-install-scaffolding/spec.md`) records an explicit "Scope inversion note": the planned `cortex setup` was found unnecessary and the ticket shipped pure retirement. Verified by inspection of `cortex_command/cli.py` — only `overnight`, `mcp-server`, `init`, `upgrade` subcommands are registered; no `setup` subparser exists. `cortex upgrade` is `git pull --ff-only && uv tool install -e --force` only — it does not touch `~/.claude/`.
