...
- **Expected**: `git diff --name-only main..HEAD` (extended to working tree via `git status --porcelain` per plan Task 4) shows only paths under `lifecycle/archive/audit-skill-prompts-and-remove-verbose-instructions-above-the-floor/` or `backlog/`. No matches under `skills/`, `hooks/`, `claude/`, `tests/`, `bin/`, or other source directories.
