...
- (d) `cortex_command/cli.py:_resolve_cortex_root` is deleted; `--print-root` returns the user's project root (via `_resolve_user_project_root()`); a new `package_root` field reports the package install location for diagnostic introspection. The JSON envelope `version` is bumped to `"1.1"` (additive: 1.0 consumers ignore the new field). Resolved per Open Decisions D2 → option (a). Verify: `grep -c "_resolve_cortex_root" cortex_command/cli.py` = 0; `cortex --print-root --format json` emits a payload with `version == "1.1"` and both `root` and `package_root` keys present.
...
- **D2 — `--print-root` semantic under wheel install** (resolved → option a). Under non-editable wheel install the user's project root and the package install location live at different paths, so a single `root` field cannot serve both diagnostic introspection and session-dir/lifecycle-dir resolution. Option (a) — `root` is the user's project root (the single-source-of-truth contract per `_resolve_user_project_root()`); a new `package_root` field surfaces the package install location for diagnostics — was selected during Task 6 implementation. JSON envelope `version` bumped from `"1.0"` to `"1.1"` (additive: legacy 1.0 consumers ignore the new field). R2d amended accordingly.

(All other research-phase Open Questions resolved or deferred-with-default before spec write; critical-review fix-invalidating findings were resolved by the spec rewrite.)
