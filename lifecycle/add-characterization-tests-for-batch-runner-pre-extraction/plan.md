...
- **Context**:
  ...
  - `feature_result_variants.py`: a plain Python module defining module-level `FeatureResult` instances, one per status. Import: `from cortex_command.overnight.batch_runner import FeatureResult`. Name them `COMPLETED`, `FAILED`, `PAUSED`, `DEFERRED`, `REPAIR_COMPLETED`. Tests in Tasks 3–7 construct `FeatureResult` instances inline; this module serves as a reference and can be imported in future tests that prefer fixture reuse.
