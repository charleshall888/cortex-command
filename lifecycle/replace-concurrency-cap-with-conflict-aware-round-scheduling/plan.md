...
- **Verification**: `python3 -c "from cortex_command.overnight.batch_runner import BatchConfig; assert not hasattr(BatchConfig, 'concurrency')"` — pass if no assertion error; `python3 -m claude.overnight.batch_runner --help 2>&1 | grep -c 'concurrency'` — pass if count is 0 (the `--concurrency` flag is gone). Note: `batch_runner.py` contains many references to `ConcurrencyManager` (imports, type annotations, docstrings) which must remain — do not use a bare `grep 'concurrency'` as verification since those legitimate references will match.
- **Status**: [x] complete
