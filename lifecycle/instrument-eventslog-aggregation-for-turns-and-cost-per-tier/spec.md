...
2. **Aggregation input sources**: the aggregator reads `lifecycle/pipeline-events.log` (active session) and all `lifecycle/sessions/*/pipeline-events.log` files (archived sessions). Per-feature `lifecycle/{feature}/events.log` files are **not** read by this aggregator — dispatch events are not written there in the production path.
   - Acceptance: `python3 -m claude.pipeline.metrics --report tier-dispatch` reads from the session pipeline log glob; `pytest claude/pipeline/tests/test_metrics.py -k pipeline_events_sources` exits 0 and verifies the glob matches the session archive layout using a hand-crafted fixtures directory.
...
5. **Windowing via `--since DATE` CLI flag**: `python3 -m claude.pipeline.metrics --report tier-dispatch [--since YYYY-MM-DD]` filters events to those with `ts >= DATE`. Defaults to all-time when `--since` is omitted. This is the mechanism for carving DR-4's post-4.7 baseline window.
   - Acceptance: `python3 -m claude.pipeline.metrics --report tier-dispatch --since 2026-04-18` exits 0 and excludes events prior to 2026-04-18; `pytest claude/pipeline/tests/test_metrics.py -k since_flag` exits 0 and verifies the filter drops pre-DATE events from the aggregate.
...
- **ADDED**: CLI flags `--report tier-dispatch` and `--since YYYY-MM-DD` on `python3 -m claude.pipeline.metrics`.
