...
- New: `lifecycle/measure-xhigh-vs-high-effort-cost-delta-on-representative-task/synthetic-task/` — a pinned synthetic task (scratch worktree state + prompt + expected acceptance criteria).
- New: `lifecycle/measure-xhigh-vs-high-effort-cost-delta-on-representative-task/harness.py` — thin Python script that calls `dispatch_task()` twice with different `effort_override` values, writes local events.log, prints summary.
- New: `lifecycle/measure-xhigh-vs-high-effort-cost-delta-on-representative-task/report.md` — the deliverable.
