...
- **JSONL** (single-line): `{"ts": ..., "event": "critical_review", "phase": "...", "applied": [...], "dismissed": [], "asked": [...]}` — e.g. `lifecycle/add-characterization-tests-for-batch-runner-pre-extraction/events.log:13`.
- **YAML multi-line block**: seen in `lifecycle/extract-optional-skills-to-plugin/events.log:63–131` — includes `dispositions: {apply:, dismiss:, ask:, defer:}` and `applied_fixes:` list, and in some variants a `dismissed:` or `deferred:` list with prose.
