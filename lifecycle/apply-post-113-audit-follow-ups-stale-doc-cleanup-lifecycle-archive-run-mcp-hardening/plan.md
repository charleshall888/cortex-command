...
- **Depends on**: [2]
- **Complexity**: complex
- **Context**: Six tool-body sites identified by `grep -n -E '_run_cortex\(|_get_cortex_root_payload\(\)' server.py`: 1417, 1485, 1493, 1586, 1666, 1739. Existing `_maybe_check_upstream` (line 466) and `_maybe_run_upgrade` (line 975, 1077, 1151) are EXCLUDED — they retain swallow-and-skip via their pre-existing `except OSError` clauses (CortexCliMissing inherits from OSError, so the existing handlers fire automatically). Concurrency: spec edge case "S1.3 concurrent dispatch under asyncio" — two coroutines may both observe FNF and clear the cache; the second's retry reads the now-primed cache without re-running discovery. Retries are idempotent under this model. Pattern: extract a small `_retry_on_cli_missing(func, *args)` helper if it cleans up the call sites, otherwise inline the try/except at each site (pick the form that keeps each call site under ~10 lines added).
...
- **Depends on**: [1, 2, 3]
- **Complexity**: complex
- **Context**: Cases (one test function per case): (a) FNF in `_get_cortex_root_payload` raises `CortexCliMissing` (monkeypatch `subprocess.run` to raise `FileNotFoundError`); (b) FNF in `_run_cortex` raises `CortexCliMissing` (same pattern); (c) `PermissionError` is NOT converted to `CortexCliMissing` (negative test — assert `pytest.raises(PermissionError)`); (d) cache-then-FNF-then-recovery succeeds with one retry (prime cache, monkeypatch FNF on first call only, verify second call succeeds and `_CORTEX_ROOT_CACHE` repopulated); (e) cache-then-FNF-then-FNF propagates an `mcp.types.McpError` whose `data.reason` == `"cortex_cli_missing"` and `data.install_url` == README anchor URL; (f) `_maybe_check_upstream` returns its existing sentinel (`None` or equivalent) on `CortexCliMissing` (asserts OSError parentage preserves swallow); (g) startup `shutil.which` returning None emits the expected stderr line (capsys check); (h) byte-equal McpError envelope from two different propagation paths (call helper directly + propagate via tool body, compare `.data` dict). Use `pytest.MonkeyPatch` and `unittest.mock.patch` per existing test patterns in `tests/test_mcp_subprocess_contract.py`. Concurrency case for (d) optional but recommended: use `asyncio.gather` on two parallel mock-tool-calls to verify convergence to single re-discovery; if asyncio harness is heavy, document the case in a TODO comment and rely on the spec's correctness argument (file-bug if convergence ever fails).
...
- **Depends on**: none
- **Complexity**: complex
- **Context**: Existing recipe behavior: idempotent, `set -euo pipefail`, iterates `lifecycle/*/events.log` and skips `lifecycle/archive/*`, `lifecycle/sessions/*`, active worktrees. New behavior: dry-run gates the destructive operations behind a single conditional; manifest write is `printf '%s\n' "$line" > "$tmpfile" && mv "$tmpfile" "$manifest"` (or `cat $manifest $tmpfile > combined && mv` for append-atomic; pick the form that survives concurrent dry-run-then-real). Precheck aborts with a clear error pointing the user to commit/stash. Spec edge case: dry-run also requires clean tree (consistency for downstream consumers). Stale-symlink guard: add `[ -e "$dir" ]` before `realpath` to skip broken symlinks gracefully (spec edge case "N6 stale symlinks").
...
### Task 12: lifecycle-archive recipe — path-rewriting (slash, wikilink, word-boundary) with .bak trap cleanup (N6.3)
- **Files**: `justfile`
- **What**: Add post-`mv` path-rewriting for three citation forms across all `*.md` under repo root excluding `.git/`, `lifecycle/archive/`, `lifecycle/sessions/`, and `retros/`. `.bak` files cleaned via shell `trap` so both success and failure paths leave no residue.
- **Depends on**: [11]
- **Complexity**: complex
- **Context**: Three regex anchored substitutions per archived slug — slug values must be regex-escaped before substitution: (1) slash form `\blifecycle/{slug}/` → `lifecycle/archive/{slug}/`; (2) wikilink form `\[\[lifecycle/{slug}(/|\]|\|)` → `[[lifecycle/archive/{slug}\1`; (3) word-boundary bare form `\blifecycle/{slug}\b` (not followed by `/`) → `lifecycle/archive/{slug}`. Implementation: `sed -i.bak -e 'expr1' -e 'expr2' -e 'expr3'` per slug, OR a single combined alternation per file. Trap: `trap 'find . -name "*.md.bak" -delete' EXIT INT TERM` set near the top of the recipe (resolves spec residue: failure path must not leave stranded `.bak` files). Per-file `rewritten_files` list populated by capturing `grep -l` matches before sed runs, fed into the manifest entry from Task 11. Spec edge cases handled: substring collision (`add-foo` vs `add-foo-bar`) — slash anchor + `\b` word boundary prevent false rewrites; nested slug references — slash-form pattern handles transitively; wikilink variants `[[lifecycle/foo]]`, `[[lifecycle/foo/]]`, `[[lifecycle/foo|x]]`, `[[lifecycle/foo/index]]` — wikilink anchor catches all four. Spec residue addressed: rewrite scope INCLUDES `docs/`, `README.md`, `SKILL.md` citers because the find/grep walks all `*.md` under repo root with only the four exclusions.
- **Verification**: After `just lifecycle-archive --dry-run` on a representative slug, `find . -name '*.md.bak' | wc -l` = 0 (trap cleaned even on dry-run if any sed ran in test mode). After full run on the sample (verified in Task 15), `grep -rn -E "lifecycle/(slug-1|slug-2|slug-3)/" --include='*.md' . --exclude-dir=.git --exclude-dir=archive --exclude-dir=sessions --exclude-dir=retros | grep -v 'archive/' | wc -l` = 0 (no unrewritten references remain). Plus integration check in Task 15. — pass if both hold.
- **Status**: [ ] pending
