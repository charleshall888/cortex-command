...
**`cortex/lifecycle.config.md` schema** — `cortex init` scaffolds this file with YAML frontmatter for project-specific overrides. The annotated, canonical field list lives in the cortex-core plugin asset `skills/build/assets/lifecycle.config.md` — a parity test (developer-run `just test`, and a blocking `validate.yml` CI step) checks its frontmatter stays byte-identical to what `cortex init` scaffolds (see ADR-0017), so that asset is the single place to read the scaffolded schema rather than re-listing it here.
