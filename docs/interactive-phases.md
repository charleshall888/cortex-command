...
**For:** Users running their first `/cortex-core:build` or `/cortex-core:refine` — or anyone wanting to understand how interactive phases work.  **Assumes:** Basic familiarity with Claude Code skills.

When you invoke `/cortex-core:build`, `/cortex-core:refine`, `/cortex-core:discovery`, or `/interview`, the agent pauses to ask questions and produce artifacts. This guide explains what each interactive phase produces, what questions to expect, and how the artifacts flow between skills. Read this before your first `/cortex-core:refine` or `/cortex-core:build` run to avoid surprises.
...
backlog tickets (cortex/backlog/NNN-slug.md)
    |
    v  (/cortex-core:refine or /cortex-core:build picks up discovery artifacts automatically)
/cortex-core:refine  (Clarify → Research → Spec)
         -->  cortex/lifecycle/{slug}/research.md
         -->  cortex/lifecycle/{slug}/spec.md
    |
    v
/cortex-core:build (plan phase)
    |
    v
...
You do not need to run all three skills — `/cortex-core:build` on a fresh feature covers the full journey. The other skills exist for partial workflows: `/cortex-core:discovery` when you have a vague idea, and `/cortex-core:refine` when you want to prepare a backlog item before overnight execution.

---

## /cortex-core:build — Full Feature Lifecycle

`/cortex-core:build <feature>` drives a feature from idea to merged code. The early phases (Clarify, Research, Specify) are delegated to `/cortex-core:refine` internally; the later phases (Plan, Implement, Review, Complete) run directly in the lifecycle context.
...
`/cortex-core:build <feature>` can be run at any time. The skill detects the current phase by checking which artifacts exist in `cortex/lifecycle/{feature}/`:
...
`/cortex-core:refine <item>` prepares a single backlog item for overnight autonomous execution. It runs exactly the same Clarify, Research, and Specify phases as `/cortex-core:build`, but stops before planning. When `/cortex-core:refine` completes, the backlog item has `status: refined` and a linked spec — the overnight runner can plan and execute it without further human input.
...
After spec approval, `/cortex-core:refine` writes `status: refined` and `spec:` to the backlog item's YAML frontmatter. The next `/overnight` or `/cortex-core:build` run picks up these artifacts automatically and skips to planning.
...
The readiness gate (and `/cortex-core:build`'s phase detection) checks for artifact file existence only — it does not assess content freshness. A `spec.md` written months ago passes the gate and is scheduled for overnight execution exactly like a freshly approved spec. There is no automatic staleness detection.
...
`/cortex-core:discovery <topic>` is for topics that are not yet concrete enough for `/cortex-core:build`. It researches the problem space broadly, then decomposes findings into backlog tickets grouped by epic. Unlike `/cortex-core:build`, discovery does not produce a plan or write code.
...
When `/cortex-core:build` starts on a ticket created by `/cortex-core:discovery`:
...
/cortex-core:discovery <topic>
    ...
    +-- cortex/backlog/NNN-slug.md            (one per ticket, discovery_source: field set)
         |
         v  (when /cortex-core:build or /cortex-core:refine picks up the backlog item)
/cortex-core:refine <item>  -- OR --  /cortex-core:build <feature>  [early phases]
    |
    +-- cortex/lifecycle/{slug}/research.md   (implementation-level research)
    +-- cortex/lifecycle/{slug}/spec.md       (approved requirements spec)
         |
         v  (/cortex-core:build plan phase, or overnight runner)
/cortex-core:build <feature>  [plan phase]
    |
    +-- cortex/lifecycle/{slug}/plan.md       (task breakdown, orchestrator-reviewed)
         |
         v  (/cortex-core:build implement phase)
Implement
    |
    +-- Source code changes
...
- `/cortex-core:build`: `skills/build/SKILL.md` and `skills/build/references/`
