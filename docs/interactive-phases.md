...
**For:** Users running their first `/cortex-interactive:lifecycle` or `/cortex-interactive:refine` — or anyone wanting to understand how interactive phases work.  **Assumes:** Basic familiarity with Claude Code skills.

When you invoke `/cortex-interactive:lifecycle`, `/cortex-interactive:refine`, `/cortex-interactive:discovery`, or `/interview`, the agent pauses to ask questions and produce artifacts. This guide explains what each interactive phase produces, what questions to expect, and how the artifacts flow between skills. Read this before your first `/cortex-interactive:refine` or `/cortex-interactive:lifecycle` run to avoid surprises.
...
```
/cortex-interactive:discovery
    |
    v
backlog tickets (backlog/NNN-slug.md)
    |
    v  (/cortex-interactive:refine or /cortex-interactive:lifecycle picks up discovery artifacts automatically)
/cortex-interactive:refine  (Clarify → Research → Spec)
         -->  lifecycle/{slug}/research.md
         -->  lifecycle/{slug}/spec.md
    |
    v
/cortex-interactive:lifecycle (plan phase)
    |
    v
...
You do not need to run all three skills — `/cortex-interactive:lifecycle` on a fresh feature covers the full journey. The other skills exist for partial workflows: `/cortex-interactive:discovery` when you have a vague idea, and `/cortex-interactive:refine` when you want to prepare a backlog item before overnight execution.

---

## /cortex-interactive:lifecycle — Full Feature Lifecycle

`/cortex-interactive:lifecycle <feature>` drives a feature from idea to merged code. The early phases (Clarify, Research, Specify) are delegated to `/cortex-interactive:refine` internally; the later phases (Plan, Implement, Review, Complete) run directly in the lifecycle context.
...
`/cortex-interactive:lifecycle <feature>` can be run at any time. The skill detects the current phase by checking which artifacts exist in `lifecycle/{feature}/`:
...
## /cortex-interactive:refine — Clarify → Research → Spec in One Invocation

`/cortex-interactive:refine <item>` prepares a single backlog item for overnight autonomous execution. It runs exactly the same Clarify, Research, and Specify phases as `/cortex-interactive:lifecycle`, but stops before planning. When `/cortex-interactive:refine` completes, the backlog item has `status: refined` and a linked spec — the overnight runner can plan and execute it without further human input.

### When to Use /cortex-interactive:refine

Use `/cortex-interactive:refine` when:
...
After spec approval, `/cortex-interactive:refine` writes `status: refined` and `spec:` to the backlog item's YAML frontmatter. The next `/overnight` or `/cortex-interactive:lifecycle` run picks up these artifacts automatically and skips to planning.

### State Resumption

`/cortex-interactive:refine` resumes at the appropriate point:
...
The readiness gate (and `/cortex-interactive:lifecycle`'s phase detection) checks for artifact file existence only — it does not assess content freshness. A `spec.md` written months ago passes the gate and is scheduled for overnight execution exactly like a freshly approved spec. There is no automatic staleness detection.

**Workaround**: If you suspect `research.md` or `spec.md` is out of date (e.g., the codebase has changed significantly since the artifact was written), delete or rename the stale file and re-run `/cortex-interactive:refine` to regenerate it. Once the new artifact is approved, the feature is ready for overnight execution again.

---

## /cortex-interactive:discovery — Research → Backlog Tickets → Epic Grouping

`/cortex-interactive:discovery <topic>` is for topics that are not yet concrete enough for `/cortex-interactive:lifecycle`. It researches the problem space broadly, then decomposes findings into backlog tickets grouped by epic. Unlike `/cortex-interactive:lifecycle`, discovery does not produce a plan or write code.
...
**Clarify** — The agent asks what you want to understand, what's in scope, and how deep to go. If you run `/cortex-interactive:discovery` with no argument, it enters auto-scan mode and reads `requirements/` to suggest gap candidates.
...
When `/cortex-interactive:lifecycle` starts on a ticket created by `/cortex-interactive:discovery`:
...
This means a well-run `/cortex-interactive:discovery` session can eliminate hours of repeated research for every downstream feature.
...
```
/cortex-interactive:discovery <topic>
    |
    +-- research/{topic}/research.md   (durable research artifact)
    +-- backlog/NNN-slug.md            (one per ticket, discovery_source: field set)
         |
         v  (when /cortex-interactive:lifecycle or /cortex-interactive:refine picks up the backlog item)
/cortex-interactive:refine <item>  -- OR --  /cortex-interactive:lifecycle <feature>  [early phases]
    |
    +-- lifecycle/{slug}/research.md   (implementation-level research)
    +-- lifecycle/{slug}/spec.md       (approved requirements spec)
         |
         v  (/cortex-interactive:lifecycle plan phase, or overnight runner)
/cortex-interactive:lifecycle <feature>  [plan phase]
    |
    +-- lifecycle/{slug}/plan.md       (task breakdown, orchestrator-reviewed)
         |
         v  (/cortex-interactive:lifecycle implement phase)
Implement
    |
    +-- Source code changes
...
- `/cortex-interactive:lifecycle`: `skills/lifecycle/SKILL.md` and `skills/lifecycle/references/`
- `/cortex-interactive:refine`: `skills/refine/SKILL.md`
- `/cortex-interactive:discovery`: `skills/discovery/SKILL.md`
