...
The kept-pause taxonomy's durable source of truth is `skills/build/references/kept-pauses-data.toml` — one row per `<!-- pause: <slug> <kind> -->` marker in skill prose. `just kept-pauses` prints a human-readable inventory from it, and the parity test at `tests/test_lifecycle_kept_pauses_parity.py` enforces marker/data set-equality — structural, and the exemplary shape for this kind of guard. (A committed rendering, `kept-pauses.md`, and its freshness check were deleted on 2026-09-16: nothing loaded it. The per-kind semantic anchor check went earlier, on 2026-08-28, as a prose-layout pin.) When modifying phase sequencing, add or remove the `<!-- pause: -->` marker in prose and update the matching row in the data file.
