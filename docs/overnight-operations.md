...
| Stall watchdog | Per-site child inactivity (no growth of the watched child signal for `STALL_TIMEOUT_SECONDS`, 1800s — the timer resets on each child-progress write, `stall_reason="inactivity"`), or the never-reset absolute ceiling (`ABSOLUTE_CEILING_SECONDS`, 14400s, `stall_reason="ceiling"`). Batch_runner watches the worker-written `pipeline-events.log`; the orchestrator watches its own stream-json stdout — both reset on real child progress. | Kills the Claude agent, pauses state, sends notification |
...
`cortex overnight schedule <target>` schedules a single overnight run at a future time without requiring tmux or a persistent shell session. Under the hood it delegates to `MacOSLaunchAgentBackend.schedule()`, which renders a plist into `$TMPDIR/cortex-overnight-launch/`, writes a paired bash launcher alongside it, and calls `launchctl bootstrap gui/$(id -u)` to register the job with launchd. At fire time launchd executes the launcher, which invokes `cortex overnight start --state <abs> --format json --force` in the foreground (no `--launchd`). The detach happens inside cortex: `start` spawns the runner via `subprocess.Popen(..., start_new_session=True)`, so the runner becomes its own session leader and survives launchd's process-group SIGTERM when the launcher exits — the launcher no longer re-implements detachment in bash. No tmux session is created or required; the runner writes its own `runner.pid` artifact and operates identically to a manually-launched session.
...
- **`caffeinate -i` does not wake a sleeping Mac.** The runner holds a `caffeinate -i` idle-sleep assertion bound to its own lifetime, which prevents the machine from going to idle sleep *while a run is in progress*. It does **not** wake a Mac that is already asleep, and it has no effect before the runner is spawned. If the Mac is asleep when the schedule fires, the missed interval coalesces and the job runs on the next wake; if the machine is powered off at fire time, the run is skipped entirely until the next scheduled time. `caffeinate` keeps an active run alive; it is not a wake mechanism (cortex does not use `pmset schedule` to wake the machine — see the no-wake limitation above).
- **The plist `StartCalendarInterval` `Year` key is inert.** Scheduled runs are one-shot, but launchd's `StartCalendarInterval` has no `Year` field, so the rendered calendar block recurs annually by date. The one-shot semantics are **not** enforced by the date — they are enforced by teardown: the runner issues `launchctl bootout gui/$(id -u)/<label>` for its own label after a successful fire, and a GC pass in `schedule()` reaps spent `scheduled-launches.json` entries (and their plist/launcher files) whose `scheduled_for_iso` is in the past, regardless of registration state. Do not rely on the calendar date to prevent an annual refire; rely on bootout + GC.
...
- **Event/heartbeat staleness (secondary, Phase 2).** For the alive-but-wedged runner (pid reads alive but the event loop is hung), recovery keys on a runner-level heartbeat older than a threshold (`> 2700s`) strictly above the in-process watchdog's `STALL_TIMEOUT_SECONDS` (`1800s`). The two layers no longer race on a shared "stall" definition: the in-process watchdog now keys on *child silence* — a per-site activity-reset inactivity timer that resets on each child-progress write, not the old blind 30-minute tick-counter — while this guardian/recovery layer keys on *parent runner-event/heartbeat staleness*. The strict-greater margin lets the runner's own watchdog attempt a graceful self-heal of a silent child first. Note the guardian watches the **parent** runner and cannot catch a hung orchestrator *child* mid-round (an active-but-doomed child whose stdout keeps advancing without real progress); the in-process child-stdout watchdog and the never-reset absolute ceiling (`ABSOLUTE_CEILING_SECONDS`, 14400s) own that case. Because a wedged runner is still alive and could overwrite `paused` → `executing`, this path SIGKILLs the create_time-verified runner before transitioning. This is the lower-priority, false-positive-prone signal; the pid-death signal already covers the dominant dead-runner mode.
...
| `cortex/lifecycle/{feature}/events.log` | Investigating phase transitions, review verdicts, and completion for one feature — what `/cortex-core:build resume` and `/morning-review` read. |
...
### Seatbelt sandbox verification

Per-spawn sandbox configuration is necessary but not sufficient: a regression in `cortex_command/pipeline/worktree.py:resolve_worktree_root()` (the single chokepoint for Seatbelt-writability per `cortex/requirements/multi-agent.md:77`) would silently break per-feature dispatch worktrees without tripping any existing gate. The kernel-probe test below closes this gap as an on-demand check.

**Kernel-probe gate.** `tests/test_worktree_seatbelt.py`'s `seatbelt_active` fixture attempts to open `<repo>/.git/HEAD` for write (`O_WRONLY`, no truncation, no creation). `.git/HEAD` is one of the suffixes denied by `build_orchestrator_deny_paths` in `cortex_command/overnight/sandbox_settings.py`, so a `PermissionError` confirms the sandbox is actually enforcing the deny rule. A successful open means no sandbox is active and the test skips with a kernel-anchored reason. This replaces the legacy `CLAUDE_CODE_SANDBOX=1` env-var gate, which was undocumented on macOS Seatbelt and silently skipped in every Claude-Code-spawned automation context — see `cortex/lifecycle/re-validate-test-worktree-seatbeltpy-on/research.md` for the full history. Run it on demand from an active Claude Code session (which is itself sandboxed) with `pytest tests/test_worktree_seatbelt.py -q`.

**Retired: automated session-start probe.** An earlier design (`cortex_command/overnight/seatbelt_probe.py`, removed) had the runner spawn `claude -p` at session start to run the kernel-probe test inside an orchestrator-style sandbox and emit a `seatbelt_probe` event to a top-level `cortex/lifecycle/seatbelt-probe.log`. It was retired because it could never pass: under an enforcing sandbox in non-interactive `-p` mode, Claude Code declines to run the compound `uv run pytest … | tee …; printf …` Bash command the probe issued (and `--dangerously-skip-permissions` does not override a sandbox-driven denial), so every logged run failed without ever executing the test. The bind was structural — the test must run *inside* the enforcing sandbox to detect enforcement, but relaxing the sandbox enough to run the test harness makes the inner test skip. The on-demand kernel-probe gate above is the supported verification; the per-spawn sandbox settings remain the production enforcement mechanism. A separately tracked test-hygiene defect rode along: the runner wrote the top-level log via a repo-relative path, so integration tests driving the real `run()` from the repo root polluted the tracked file with test-session noise.
