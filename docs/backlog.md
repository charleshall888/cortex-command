...
**Gate 2 — Blocked.** The item's `blocked-by` list must not contain any item that is itself in a non-terminal status. If any blocker is still active, the item is ineligible. Terminal statuses that satisfy this gate are: `resolved`, `wontfix`, `done`, `complete`, `abandoned`.
...
**Additional side effects for terminal status transitions** (`complete`, `abandoned`, `done`, `resolved`, `wontfix`):
