...
> **`type: epic` gates implementability, not grouping.** This gate is the one place `type: epic` is load-bearing. It does *not* decide who a ticket's children are: `cortex-build-epic-map` groups under any id another item names as its `parent`, whether or not that parent is typed `epic`. Restricting grouping to typed epics dropped the relationship silently — declared in frontmatter, absent from the envelope, no error — which on this repo hid 14 tickets across 7 parents, one of them (#357, typed `chore`) with eight children. A ticket that heads a group is still selectable work unless its own `type` says otherwise.
