...
### critical-review
Derives challenge angles from the artifact and project context (default 2, escalating to 3-4 on high/critical criticality or novel claims), then dispatches one reviewer agent per angle in parallel for deep, unanchored criticism. An Opus synthesis agent merges the parallel findings into a single coherent challenge. Also auto-triggers in the lifecycle for Complex + medium/high/critical features before spec approval.
