...
| `plugins/cortex-core/hooks/cortex-lifecycle-continue.sh` | Stop | Resume the build loop when it ends a turn at a non-pausing phase (`review`, `complete`) of the lifecycle bound to *this* session. Exits on a single directory test where no lifecycle exists. Nudges once per unit of real lifecycle progress (keyed on state + event count), so conversational turns are never hijacked; hard backstop at 10 per feature. Disabled per-lifecycle by `touch cortex/lifecycle/<slug>/.autocontinue-off` or globally by `CORTEX_NO_AUTOCONTINUE=1` | Claude only |
