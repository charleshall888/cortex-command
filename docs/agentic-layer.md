...
|-------|---------|----------|----------|---------------|
| **dev** | Routing hub for all development requests | `/dev`, "what should I work on", "next task" | Routes to lifecycle/overnight/discovery/direct | All agents |
...
Cross-agent hooks run in both Claude Code and Cursor. Claude-only hooks run only in Claude Code sessions. The `claude/hooks/` subdirectory contains hooks specific to Claude Code's permission and session model.

| File | Event | Purpose | Agents |
|------|-------|---------|--------|
| `hooks/validate-commit.sh` | PreToolUse (Claude), beforeShellExecution (Cursor) | Validate commit message: imperative mood, ≤72 chars subject, no trailing period, blank line before body | Both |
| `hooks/scan-lifecycle.sh` | SessionStart | Inject `LIFECYCLE_SESSION_ID`, active feature state, overnight execution state, and fresh-resume prompts into context | Both |
| `hooks/notify.sh` | Stop, Notification | Desktop notifications via terminal-notifier when Claude needs input or completes (macOS); also registered in Cursor's stop hook | Both |
...
3. **validate-commit hook** — Pre-execution gate on all `git commit` commands, active in both Claude Code and Cursor. Enforces imperative mood, ≤72-character subject line, no trailing period, and a blank line before the body. Prevents malformed commit messages from entering the repo regardless of which agent created them.
