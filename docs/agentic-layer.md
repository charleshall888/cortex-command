...
graph TD
    ...
    MR["/cortex-overnight:morning-review\nanswer deferrals\nadvance lifecycles\nmerge PR → main"]
...
Research fan-out is always parallel and sized by a tier×criticality matrix (range 3–10) — see [`skills/lifecycle/references/fanout.md`](../skills/lifecycle/references/fanout.md). Competing/parallel plans remain critical-only.
...
When multiple backlog items are ready, the user runs `/cortex-core:refine` per feature to produce `research.md` and `spec.md` for each, then `/cortex-overnight:overnight` to plan and execute them in a batch. The overnight runner creates git worktrees (one per feature) *(WorktreeCreate hook fires here, setting up branch isolation for each worker)*, dispatches feature workers using the `cortex_command/pipeline/` execution module, and merges results into an integration branch. `/cortex-overnight:morning-review` closes the loop — reading the overnight report, closing completed lifecycles, and surfacing any features that need follow-up.

### 3. Autonomous Overnight

In the evening, the user runs `/cortex-overnight:overnight` to plan a batch of features for unattended execution *(SessionStart hook: `plugins/cortex-overnight/hooks/cortex-scan-lifecycle.sh` fires here, injecting `LIFECYCLE_SESSION_ID` and active feature state into context so the session begins oriented to current work)*. **Prerequisite**: selected features must already have discovery artifacts (`research:` and `spec:` fields in their backlog YAML frontmatter) — `/cortex-overnight:overnight` does not run interactive research or spec phases. The plan lists the eligible features and estimated duration; after user approval, the runner detaches in a tmux session and begins working. Through the night, the runner selects features from the approved batch, creates branches, and runs lead agents with a tier-based conflict-aware scheduling system to avoid resource contention. Each feature picks up at the plan phase (or implement, if already planned). In the morning, `/cortex-overnight:morning-review` walks the overnight report: it reads `cortex/lifecycle/morning-report.md`, closes completed lifecycles, merges approved PRs, and surfaces any features that need follow-up. For the full architecture and operational guide, see [Overnight: In Depth](overnight.md).
...
|------|-------|---------|--------|
| `plugins/cortex-core/hooks/cortex-validate-commit.sh` | PreToolUse | Validate commit message: imperative mood, ≤72 chars subject, no trailing period, blank line before body | Claude only |
| `plugins/cortex-overnight/hooks/cortex-scan-lifecycle.sh` | SessionStart | Inject `LIFECYCLE_SESSION_ID`, active feature state, and overnight execution state into context | Claude only |
...
| `plugins/cortex-overnight/hooks/cortex-cleanup-session.sh` | SessionEnd | Remove `.session` lock files from `cortex/lifecycle/*/` when a Claude Code session ends (skips on `/clear`) | Claude only |
| `plugins/cortex-overnight/hooks/cortex-permission-audit-log.sh` | Notification (permission_prompt) | Append one line per permission prompt to a session-scoped log in `$TMPDIR` for sandbox tuning diagnostics | Claude only |
| `plugins/cortex-overnight/hooks/cortex-tool-failure-tracker.sh` | PostToolUse (Bash) | Track Bash tool failures by exit code; surface a warning via `additionalContext` after 3 failures for the same tool in one session | Claude only |
| `claude/hooks/cortex-skill-edit-advisor.sh` (unregistered) | PostToolUse (Write\|Edit) | Advise on skill editing best practices when a Write or Edit touches a file inside `skills/` | Claude only |
| `plugins/cortex-core/hooks/cortex-worktree-create.sh` | WorktreeCreate | Create a git worktree with branch isolation for parallel overnight or feature work | Claude only |
| `plugins/cortex-core/hooks/cortex-worktree-remove.sh` | WorktreeRemove | Clean up the worktree directory and merged branch after work completes | Claude only |

### Hooks Architecture

#### Event Types

Claude Code fires hooks at six lifecycle points, each corresponding to a registered key in each plugin's `hooks/hooks.json`:
...
- **`"updatedInput"`** (PreToolUse only) replaces the tool's input before execution. The object must match the tool's input schema (e.g., `{"command": "..."}` for Bash). Hooks that ship in this repo do not currently use `updatedInput`; the field is documented here for completeness.
...
- **`SessionStart`** — `{"cwd": "/path/to/project", "session_id": "...", ...}`. Used by `plugins/cortex-overnight/hooks/cortex-scan-lifecycle.sh` to discover active lifecycle features under the session's repo root.
...
4. **Backlog index** (`cortex/backlog/index.md`) — Generated by `/cortex-core:backlog reindex`. `/cortex-core:dev` reads it during triage to identify ready work. Items are auto-closed by `/cortex-core:lifecycle complete` and `/cortex-overnight:morning-review`, keeping the index current without manual intervention.
