...
| [`measure-tokens.py`](../../cortex/lifecycle/requirements-skill-v2/scripts/measure-tokens.py) | `requirements-skill-v2` (Task 15) | Measure cl100k_base token count of a file (default: `cortex/requirements/project.md`). Iteration aid for the Phase 3 trim loop; no production code depends on it. | one-shot complete (Phase 3 trim landed; retained as historical documentation) |
| [`verify-conditional-loading.py`](../../cortex/lifecycle/requirements-skill-v2/scripts/verify-conditional-loading.py) | `requirements-skill-v2` (Task 16, R12) | Cross-phase intersection check: for each area-doc stem (`multi-agent`, `observability`, `pipeline`, `remote-access`), assert that the corresponding Conditional Loading trigger in `cortex/requirements/project.md` matches at least one real tag word found in an active lifecycle `index.md`. Fails only on drift, not coverage gaps. | one-shot complete (Task 16 verified post-trim; retained as drift-detection reference) |
| [`post-trim-anchor-check.sh`](../../cortex/lifecycle/requirements-skill-v2/scripts/post-trim-anchor-check.sh) | `requirements-skill-v2` (Task 15, R10/R11) | Verify every anchor enumerated in `trim-anchor-audit.md` is still resolvable in the trimmed `cortex/requirements/project.md`. Exits 0 on all-pass, 1 on any miss. Not wired into the runner. | one-shot complete (post-trim verification ran during Task 15) |
