...
| `cortex/lifecycle/pipeline-events.log` | Append-only JSONL event log. Each line is a timestamped event (dispatch_start, dispatch_complete, merge_start, merge_success, etc.). Note: written per-session under `cortex/lifecycle/sessions/{id}/`. |
...
keys are `could_not_run`, `merge_reverted`, and `review_dispatch_crashed`; `review_no_artifact`
is a cause-class value.
