...
const NOTES = {
  ...
  "sc-parts": {
    beats: [
      ...
      "(four strands fan into a diamond; only two come out the other side) \u201cSomething has to decide what goes in there. That thing is the harness \u2014 Claude Code, Cursor, Codex. Whichever one you\u2019re in, that\u2019s its whole job.\u201d Count it out loud \u2014 it\u2019s the whole picture: \u201cfour things went in. Two came out.\u201d Then: \u201cthe rest of what you wrote exists. It just didn\u2019t make the cut this turn.\u201d IF ASKED where the model ends and the harness begins: \u201cthe window is the model\u2019s. What\u2019s in it is the harness\u2019s \u2014 and what the harness gets to choose from is yours.\u201d",
      ...
      "(the slab grows and gains layers; the diamond flattens) \u201cThe model takes more on with every release \u2014 and there\u2019s less for the harness to sort because of it. That part isn\u2019t your doing: you didn\u2019t build it, and you can swap it out.\u201d THE EVIDENCE (the note line): \u201cthis isn\u2019t hypothetical. When Opus 5 shipped, Anthropic cut about eighty percent of the prompt guidance out of Claude Code \u2014 the model had outgrown it.\u201d Notice what didn\u2019t move: \u201cthe window. Same size it always was.\u201d",
  ...
  "sc-prism": {
    beats: [
      ...
      "(the sea folds; head + cloud) “So I spend that first 20% deliberately, and it always starts the same way — the interview. The epic — still just a cloud in my head.”",
      ...
      "(doc + intent) SILENT — let “for looking back on catches” land. “The doc survives; your context doesn't.”",
    ],
    exit: "“That doc is always step one. What comes next depends on what you value.”",
  },
  "sc-roads": {
    beats: [
      "(the rail forks three ways) “Same start every time — the interview. Then three roads. Top: I work it by hand. Bottom: I hand it over and walk away. Which one depends on what I value that day — my time, or my tokens. Middle first; it's my default.”",
      "(the outer roads step aside; tickets, stamps fly) “Discovery splits the epic into three, each carrying one line of intent. Last ticket — the cold-open ask. Never a one-off; a shard of a plan. You see it run end-to-end at the finale — here, just the shape.”",
      "(tickets fold away; the bottom road lights) “The bottom road. Same doc — but I also hand over how to check it's done. That list is what lets me walk away.”",
      "(the long run plays — let the first two subagents run SILENT) “Top bar: the main agent — the flagship model, a 1M window, and pricey. It doesn't do the work in that window. Every piece goes to a cheaper subagent with a fresh window, and only a short result comes back. That slows the fill — it doesn't stop it. A real long run still uses 700k-plus, and a longer one auto-compacts mid-run and keeps going. And I can check in whenever I want.”",
      "(the red loop, the red chunks) “Here's the catch. It asks me less, so it guesses more — it lands near what I wanted, not on it. I expect to redirect it and rework at the end, and that's more tokens.”",
      "(the top road lights; six short turns — let two run SILENT) “The top road. Same picture, read differently. One window, short turns, and I'm at every one. Turn two I tell it something it couldn't know. Turn four it goes wrong — “I catch it at the next turn,” and it cost one turn, not a night. Everything from the first act still applies here: watch the gauge, rewind.”",
      "(the rail steps off; the table) “Weighed. By hand: the most of me, the fewest tokens — if I keep the window lean. That’s for unknown ground, code full of landmines, the foundation of a long project. Tickets sit between — most planned work, much of it automated, I approve each piece. The long run: the least of me, the most tokens — fine when the agent can check its own work, like a straight language migration. And the line underneath is the one I'd keep: “the more I'm in it, the more of what's in my head reaches the project.””",
    ],
    exit: "“So how do I pick? By how much rests on the piece.”",
  },
  "sc-trunk": {
    beats: [
      "(the trunk alone, named) “People ask how I keep the slop out. It starts with one question about every piece: “what rests on this?” In any project it's the same short list — the data model, the architecture, and the key product decisions — what this thing is for. That's the foundation — I'll call it the trunk.”",
      "(limbs draw, leaves land) “Everything else grows on top. Limbs are features — the catch log, the shop; a few things rest on them. Leaves are the empty state, the sort order, the button copy. Nothing rests on a leaf. Agents build these fast — and they pass the same tests as everything else.” DON'T say “lower quality.”",
      "(one leaf reddens, drops, regrows — SILENT till the new one lands) “Get a leaf wrong — “one ticket to redo.” Nothing else moved.”",
      "(the trunk reddens, the crack climbs, the red runs up — SILENT till the leaves droop) “Get the trunk wrong and it's every screen, plus every save file players already have. And you find out from the leaves. Flip it: “when the trunk is sound, a bug up top is a local bug.””",
      "(the canopy becomes one outline; the trunk lights) “Branches grow, fall, get trimmed — I don't track every one. “But I can explain the trunk,” because I was in every decision. Involved doesn't mean I typed it — I made its decisions and I read that one line by line. Agents start fresh every time; I'm the one who remembers why it's shaped this way.”",
      "(each part's road takes its examples' slot) “So the trunk gets the most of me — by hand, or tickets I read closely. Limbs: tickets, or the long run. Leaves: the long run is fine. First word on each line is my default. It's a guide — and once other things start resting on a leaf, it's a limb now: go back and look at it.” Let the closing line land. IF ASKED what counts as trunk: “count what reads it, and ask whether changing it means migrating data.”",
    ],
    exit: "“The trunk gets my attention — and it also gets a second pair of eyes. A fresh window has one more use: point it at your own work.”",
  },
  "sc-arrows": {
    beats: [
      ...
      "(finding 1 struck out) “It wants the log page rewritten in Rust. For a page you flip through. No.”",
  ...
  "sc-buoys": {
    beats: [
      ...
      "Let “it only fixes the ones it catches being wrong” land. Then: “the ones it never notices just sit there — steering every session. And a wrong marker glows just as bright as a right one.”",
