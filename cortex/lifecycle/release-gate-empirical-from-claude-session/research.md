...
- Note: top-level `lifecycle/pipeline-events.log` IS in the auto-resolve list. The procedure's smoke pipeline-events.log lives at `lifecycle/<smoke-slug>/pipeline-events.log` (per-feature path), which IS in the auto-resolve glob `lifecycle/*/...|pipeline-events.log`. So if any of that file lingers in post-merge sync window, it could be auto-resolved with `--theirs` (remote wins). The archive destination (not in the glob) is safe.
