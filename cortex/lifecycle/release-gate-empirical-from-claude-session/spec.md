...
7. **Wire the lint into the existing test suite without bespoke configuration.** The lint is a pytest test in `tests/` — it inherits the project's existing `pytest` discovery and `just test` recipe. No new justfile recipe required; no CI workflow changes required. Acceptance: `uv run pytest --collect-only -q tests/test_backlog_grep_targets_resolve.py 2>&1 | grep -c "test_"` returns `≥ 3` (three tests collected via standard pytest discovery — proves the lint is discoverable by the same path `just test` uses). **Phase**: Phase 2.
