...
8. **The clause distribution is greppable without new tooling.** Acceptance: the following returns per-clause counts for `criticality_override` rows only, across the whole corpus:
    ...
    **Superseded:** this form aborts on the real corpus (43% of `events.log` lines are legacy YAML). The working recipe is the one in `cortex/adr/0036-ceremony-relief-is-not-taken-on-the-criticality-axis.md` § Reading the clause distribution — see #452 review cycle 1.
