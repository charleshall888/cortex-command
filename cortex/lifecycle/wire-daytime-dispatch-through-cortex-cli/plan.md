...
- **What**: Per spec R15: after `cortex daytime start --feature smoke-detached`, assert `os.getpgid(returned_pid) != os.getpgid(os.getpid())` (the spawned child is in a different process group). Explicitly does NOT claim this proves Seatbelt escape — backlog ticket #230's manual release gate is the load-bearing empirical verification. Test asserts only PGID detachment.
- **Depends on**: [7]
- **Complexity**: simple
...
<!-- Task 17 (manual release-gate) was split out into backlog ticket #230 per Angle-3 critical-review finding: the cortex pipeline's complexity enum is closed (`trivial|simple|complex`) and `cortex_command/pipeline/dispatch.py:231-233` raises `ValueError` on unknown tier. A task with `Complexity: interactive` would crash every autonomous dispatch. The release-gate procedure now lives at `cortex/backlog/230-release-gate-empirical-from-claude-session-smoke-test-for-228-daytime-dispatch.md` and gates the production release tag (not the lifecycle's `feature_complete` event). The implementation PR should merge with a `[release-type: skip]` marker; the version tag fires only after #230 is marked merged. -->
...
- Lifecycle reaches `feature_complete` autonomously after Task 16; the implementation PR merges with `[release-type: skip]` so the auto-release workflow does not cut a version tag until backlog #230 is marked `merged` (operator runs the release-gate procedure and pushes a follow-up `[release-type: minor]` commit).
