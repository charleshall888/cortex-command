...
3. **The doc cites the seven previously-uncited lifecycle ADRs.** Acceptance: `grep -c` for each of `0010`, `0012`, `0017`, `0018`, `0020`, `0022`, `0030` in `cortex/requirements/lifecycle.md` returns ≥1. Each citation states in one line what the ADR ratifies. Grounding: `cortex/adr/`. **Phase**: Reachable doc
...
7. **Five contested items stay in `project.md` in full.** Acceptance: `project.md` still contains the complete text of the bullets currently at lines 38 (`Critical-review gates at spec only`), 40 (`The short road`), 59 (`Lifecycle identity is the canonical slug`), 61 (`Lifecycle phase vocabulary`), and 65 (`The lifecycle events corpus is mixed-format`). Rationale: 38 is dispatch policy `multi-agent.md` owns; 40 duplicates `glossary.md`, which loads unconditionally; 59, 61, and 65 carry normative clauses with real cross-area readers and would go dark for any feature whose tags miss the trigger. **Phase**: Relocation and guard
