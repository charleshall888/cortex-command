...
- **Context**:
  ...
  - `_recover_stale(feature: str, worktree_path: Path) -> None`: aborts any in-progress merge (if `MERGE_HEAD` exists in the worktree), removes all `.lock` files under the worktree, force-removes the worktree via `git worktree remove --force --force`, then prunes the worktree list via `git worktree prune`; all git subprocess calls pass `cwd=worktree_path` (not `-C`); see the SIGKILL recovery sequence in `lifecycle/archive/build-daytime-pipeline-module-and-cli/research.md` for the full rationale and ordering
