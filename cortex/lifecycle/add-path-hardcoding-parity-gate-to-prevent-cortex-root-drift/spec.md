...
2. **Detection patterns — slash-prefix literals**: The gate flags any string-literal occurrence matching the regex `["'](lifecycle|backlog|research|requirements)/` in scanned files. Acceptance: a fixture file containing `x = "lifecycle/foo/bar"` placed under `tests/fixtures/path_hardcoding/violation_slash.py` and scanned via `bin/cortex-check-path-hardcoding --root tests/fixtures/path_hardcoding --audit` exits non-zero and stderr names the file + line. **Phase**: Phase 1.
