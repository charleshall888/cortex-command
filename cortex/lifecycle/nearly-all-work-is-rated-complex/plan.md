...
- **Complexity**: trivial
- **Verification**: `awk '/^2\. \*\*Complexity\*\*/,/^3\. \*\*Criticality\*\*/' skills/refine/references/clarify.md | grep -c 'critic'` → `0`; same slice `| grep -c 'next tier down'` → `1`; `grep -c 'When torn, take the lower tier' skills/refine/references/clarify.md` → `1`; `grep -ciE 'quota|target (rate|distribution|share)|aim (for|at)' skills/refine/references/clarify.md` → `0`. Pass = all four.
- **Status**: [ ] pending
