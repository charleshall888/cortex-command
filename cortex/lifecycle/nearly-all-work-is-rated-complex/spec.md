...
- **Hard byte budget, measured.** `skills/refine/references/` sits at 20583 against a 20588 pin — 5 bytes of headroom. Deleting the calibration clause frees **89 bytes**, so the sentence R3 adds must be **≤ 94 bytes** or R3 and R6 are jointly unsatisfiable. A conforming example at 73 bytes: "State whether the next tier down was considered, and why it was rejected." Run Phase 1 before Phase 2 so the ratchet stays green mid-change, and re-run `just ratchet-refs` afterwards to lock in the lower floor.
