...
**Kept user pauses come in two kinds**: (a) `AskUserQuestion`-site pauses where a phase blocks for an interactive answer; (b) phase-exit pauses where a phase exits cleanly and waits for the user to re-invoke after performing an out-of-band action (e.g., merging a PR on GitHub). The `skills/lifecycle/references/kept-pauses.md` kept-pauses inventory and `tests/test_lifecycle_kept_pauses_parity.py` enforce both kinds.
...
- **Backlog `grep -c` resolution**: `grep -c "<token>"` Done-When/acceptance checks must reference tokens in `bin/.events-registry.md` or literal strings under `cortex_command/`, preventing acceptance criteria from passing trivially against hallucinated event names. Enforced by `tests/test_backlog_grep_targets_resolve.py`.
