...
- **Acceptance criteria**:
  - After sync completes successfully, local main is not behind remote: `git rev-list HEAD..origin/main --count` = 0. The other direction is deliberately not promised — the sync's up-to-date early exit returns success without pushing, so local-ahead commits routinely survive a successful run (`cortex-morning-review-push-closures` carries its own push for exactly this reason)
...
- **Dashboard access**: The web dashboard is unauthenticated by design; its bind address and exposure model are owned by the observability area (see `cortex/requirements/observability.md`).
...
- **TMPDIR cleared mid-session**: The home integration worktree is re-created in place at `state.worktree_path` on next access. When re-creation is impossible, the feature is routed to `deferred` with `error: "integration worktree unresolved"` and **no** `recoverable_branch` — the merge was never attempted, so it neither auto-retries nor feeds the systemic breaker (same genuine-fault-vs-infrastructure-fault split as `cortex/adr/0015-review-could-not-run-vs-dispatch-crash-split.md`). The morning report names the branch to verify rather than advising a re-run
