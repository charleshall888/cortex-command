...
- **Acceptance criteria**:
  ...
  - Intra-feature task ordering is preserved: the runner derives batch ordering via `compute_dependency_batches`, keyed on each task's canonical `task_id` (the integer task number plus an optional single lowercase sub-task suffix, e.g. `3a`), from per-task `Depends on` metadata, so a task never dispatches before its declared prerequisites and letter-suffixed `### Task Na` sub-task decompositions order correctly. Genuinely-unparseable ordering metadata still fails the feature loudly rather than degrading silently — an unparseable plan raises `parse_error`, and a dependency reference that resolves to no declared task is a counted feature `failed` naming the offending id — but a supported `### Task Na` sub-task heading parses and is not unparseable. The fail-forward model governs whole-feature units, not silent mis-dispatch of tasks within a feature
...
- **Acceptance criteria**:
  ...
  - On a substantive non-APPROVED outcome (REJECTED, or CHANGES_REQUESTED after rework), the feature's live merge commit is reverted SHA-anchored under `ctx.lock` before deferring, so no unreviewed code remains on the integration branch; the one exception is a dependent-conflict revert that aborts and surfaces as a blocking deferral naming the dependent feature. The verdict-ERROR outcome is split by whether the review *agent* itself ran: a **genuine dispatch crash** (`DispatchResult.success == False` / a raised exception) reverts the merge and feeds the systemic circuit breaker as a `review_dispatch_crash` cause class; a **could-not-run review** (the agent completed `success == True` but produced no parseable verdict — file missing, no JSON block, unparseable, or a non-canonical `verdict` value) **preserves** the merge on the integration branch (`merge_reverted=False`, flagged by the positive `could_not_run` discriminator), surfaces it on the morning report AND on the integration PR as an unreviewed-merge-preserved deferral for human re-review, and feeds the systemic breaker under a distinct `review_no_artifact` cause class. Review gating applies uniformly at every merge-to-`merged` site (primary, post-recovery re-merge, and the repair_completed ff-merge), with the revert-skip guarded per-site. The systemic breaker counts crash + no-artifact failures in **aggregate** against `SYSTEMIC_FAILURE_THRESHOLD` (a mixed batch still trips), the two cause-class labels distinguishing the kinds for diagnosis only. Rationale and the safety-boundary relocation (unreviewed code may remain on the integration branch — not `main` — in the could-not-run case) are recorded in the review-could-not-run-vs-dispatch-crash-split ADR under `cortex/adr/`; the module-internals view is in `docs/internals/pipeline.md`.
- **Priority**: must-have
