...
The kept user pauses are a marked taxonomy of the deliberate, in-scope user-facing pauses across the lifecycle and refine skills. Each pause carries a `kind` — `question` (a phase blocks for an interactive answer), `phase-exit-wait` (a phase exits cleanly and waits for the user to re-invoke after an out-of-band action, e.g. merging a PR on GitHub), `config-conditional` (a pause a config key can suppress), or `relayed-consent` (a substantive approval surface whose consent overnight relays pre-authorized) — plus an orthogonal optional `suppressed_by` (a `lifecycle.config.md` key, or `judgment` for model-conditional rendering). The durable source of truth is `skills/build/references/kept-pauses-data.toml` (one row per `<!-- pause: <slug> <kind> -->` marker in skill prose); `cortex-generate-kept-pauses` prints a human-readable inventory from it, and `tests/test_lifecycle_kept_pauses_parity.py` enforces marker/data set-equality.
...
- **Override-reason clause vocabulary**: the closed set an override `reason` may lead with (`reversibility`, `exposure`, `consequence`, `other`) is owned by `cortex_command/override_reason.py` — a stdlib-only leaf beneath both writers, because `refine.py` already imports `lifecycle_event` and the shared predicate cannot sit in either without a cycle — and restated for authors in `skills/refine/SKILL.md` (both its Step 3 and Step 4 statements), `skills/build/SKILL.md`, and `cortex/adr/0036-*.md`; adding a tag edits all four. A tag is *claimed* only when the text before the first colon is a single non-empty whitespace-free token, and the stored value lowercases it; every other shape (no colon, an empty prefix, a multi-word prefix) is untagged prose written verbatim and never rejected. The two ships are ordered by direction: widening the set goes wheel-first, since no sibling repo pins a cortex-command version so the wheel reaches every consumer at once while the plugin prose ships separately via `/plugin install` — a widened wheel merely accepts a tag no prose emits yet. Conversely, narrowing goes prose-first for the same asymmetry: a narrowed wheel would start rejecting tags the installed prose still tells authors to write. The two **typed** writers of an override row — `lifecycle_event.py`'s `criticality-override`/`complexity-override` verbs and `refine.py`'s `reconcile-clarify` — validate the tag before any row is built, order their shared keys `from, to, reason` (`refine.py` appends its own `gate`), and drop `reason` entirely rather than writing a null or `""`, which names no axis a corpus tally can bucket on. The untyped `cortex-lifecycle-event log --event <e> --set k=v` escape hatch (ADR-0020, operator req 7) applies no field validation and can still record `"reason": ""` — that is the hatch behaving as specified, not a defect. → ADR-0036 (accepted).
