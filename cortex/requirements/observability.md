...
- **Description**: A FastAPI web application at `http://localhost:$DASHBOARD_PORT` (default 8080) that renders live session and pipeline state via HTMX polling. It reads session state only and never modifies it; the one thing it writes anywhere is a governing document, through the Docs view's hash-locked edit form, and it performs no git operation when it does. The panel inventory is owned by `docs/dashboard.md` and is not enumerated here.
- **Inputs**: `~/.local/share/overnight-sessions/active-session.json` (session pointer), and per-session `overnight-state.json`, `overnight-events.log`, `pipeline-state.json`, `pipeline-events.log`, `metrics.json`; `cortex/lifecycle.config.md`; `cortex/backlog/*.md` and `cortex/backlog/archive/*.md` (read only when `resolve_backlog_backend(root) == "cortex-backlog"`); per-feature `events.log`, `plan.md`, `agent-activity.jsonl`, `escalations.jsonl`, `exit-reports/*.json`, `pr.json`, `learnings/progress.txt`; the governing set read per request by the Docs view — `CLAUDE.md`, `docs/policies.md`, `cortex/lifecycle.config.md`, `cortex/README.md`, `cortex/requirements/*.md`, `cortex/adr/NNNN-*.md` — plus `cortex/backlog/` and `cortex/lifecycle/*/{research,spec,plan,review}.md` for its cited-by panel
- **Outputs**: Live HTML UI updated via HTMX at ~5s intervals; alerts surfaced in the UI and written to the dashboard process log (no external dispatch); one governing document rewritten in place per accepted edit-form save
- **Acceptance criteria**:
  ...
  - **Edit conflict**: a Docs view save whose content hash no longer matches the file on disk returns 409 and writes nothing; the response carries the operator's text, the disk version and the fresh hash, so the only way to overwrite a concurrent change is a second, informed save
- **Priority**: must-have
