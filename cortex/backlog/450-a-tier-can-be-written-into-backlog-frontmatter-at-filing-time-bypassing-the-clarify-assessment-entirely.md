...
- Some tickets legitimately carry a tier at `status: backlog`, so presence of a tier at `backlog` status is **not** the detector. Neither is absence of a `complexity_override` event, which this bullet asserted until #477 measured it: a clarified-and-parked ticket does *not* reliably carry a matching event. Clarify writes the tier back at refine Step 2, while the only in-lifecycle emitter — `refine.py`'s `reconcile-clarify` — runs at Step 4, so every lifecycle stopping before Specify holds an assessed tier with no event. Measured 2026-08-11 over `cortex/lifecycle/**/events.log` including `archive/`: of 315 logs carrying a JSON `lifecycle_start` row, **159 carry no `complexity_override`**, and 119 of those sit at `tier: complex` (count is JSON-rows-per-file; a bare `lifecycle_start` grep returns 331, the extra 16 being legacy YAML-block rows). `lifecycle_start`'s `seeded` key (`refine.py:551`) separates a defaulted tier from a frontmatter-derived one, but not an assessed tier from an unearned one — and it is present on only 19 of 330 rows. **No event-absence detector is available.** Identifying the filing channel, this ticket's stated first task, remains the only route.
