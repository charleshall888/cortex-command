...
4. ~~**`isSidechain` is dead.**~~ **This claim is itself wrong — see the correction box above.** `isSidechain` is `True` on >99.9% of subagent assistant records and is a valid splitter. The *contract* below still stands (split by **file separation**: `<session>.jsonl` vs `<session>/subagents/*.jsonl`) — file-location splitting is fine and simpler; only the justification was false. Do not build the splitter off this item's original reasoning.
...
> **CROSS-REFERENCES ARE SPENT (2026-07-17).** Every sibling this section names has since gone terminal: #381 `superseded`, #383 `superseded`, #382 / #389 / #391 `complete`, #390 `done`. Read the bullets below as history, not as live coordination. What actually shipped: **#382** landed as mechanism 2 ONLY (a `measurement` envelope field + envelope-as-sole-return), reframed as a correctness fix worth ~zero tokens — the "all four together ≈ 5%" sizing below never shipped and cannot be re-derived from it. **#389** landed as a **prose** ~40-turn cap in the dispatch prompts, with no harness enforcement. **#391** deleted the zero-reader events.
>
> **This ticket now has a named consumer it lacked when it was demoted.** The demotion rationale was "this verb gates nothing" plus "re-measurement follows shipped cuts rather than preceding them" — and the shipped cut now exists. #389's cap is unverified with **n=0** runs: nothing yet shows whether a prose cap binds. The measurement that would answer it — does the >60-turn tail (2.9% of agents, 19% of fan-out spend) actually thin against the 1,458-transcript baseline? — is the first real use for this data. **Priority stays `low` deliberately**: `cortex/research/token-economics-2026-07-16/analyze.py` can answer that question today without building the verb, so this still gates nothing. It is now a known-useful build rather than a speculative one.
