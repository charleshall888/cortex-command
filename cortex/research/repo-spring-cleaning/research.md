...
| `claude/statusline.sh` + `statusline.ps1` | **KEEP** | Forker-manual-wire path documented at `docs/setup.md:274-282`. Audience-noise but installer can skip. |
...
- `tests/test_hooks.sh:282-end-of-sync-block` (8+ test cases) + `tests/fixtures/hooks/sync-permissions/` — orphaned by `cortex-sync-permissions.py` deletion. Now paired-delete in #168.
