...
4. Is the morning-review Step-4 auto-close a real bug? → **Yes. `SKILL.md` Step 4 runs a full pre-merge auto-close [skills/morning-review/SKILL.md], but `walkthrough.md` §5 states closure "moved to §6b… closing tickets before confirming the PR has merged was a bug" [skills/morning-review/references/walkthrough.md]. The model receives two contradictory orderings of a destructive action.**
...
- **morning-review** Step 4 (`SKILL.md`) is ordered before Step 5 commit and Step 6 PR-merge, while §6b is the post-merge closer the protocol moved to [skills/morning-review/references/walkthrough.md]. §6b's own skip-guard means a pre-merge Step-4 close reintroduces the exact bug §5 says was fixed — Step 4 is stale, not a fallback.
- **dev/SKILL.md** Step 1 classifies into five first-match branches; only Branch 1 ("what's next") reaches Step 3 triage, yet the entire ~18KB SKILL.md — including the ~5KB Step-3 triage logic and the Step-2 criticality heuristic table [skills/dev/SKILL.md] — loads on all five branches.
