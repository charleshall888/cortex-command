...
Inserts `<!-- cortex-managed: lifecycle-worktree-auth version=N -->` block carrying the authorization clause body. **What it does:** satisfies the live `EnterWorktree` tool schema's project-instructions gate so that `/cortex-core:lifecycle` can auto-enter interactive worktrees without firing a per-call picker. **What breaks if removed:** every `EnterWorktree` invocation falls back to the `cd`-shim path [skills/lifecycle/references/implement.md:83]. The cd-shim still works, but the auto-enter benefit collapses: `EnterWorktree`'s session-state cache-clear (system prompt sections, memory files, plans directory) does not fire, so the orchestrator session retains stale context after entering the worktree [implement.md:74]. **Load-bearing for the steady-state `branch-mode: worktree-interactive` default**; convenience for users who haven't opted into that mode.
