...
> **Revision note 3 (direction change: DR-1 → DR-G — SUPERSEDED BY NOTE 4)**: After committing DR-1 as #195 and adding the demote-decompose option to Feasibility as cut Approach G, critical re-examination of the cut rationale found it thin: the "requires major new refine capability" framing was wrong (a small new promote-piece workflow suffices, refine is unchanged), the "cross-ticket coherence moves to a worse layer" framing was wrong (coherence lives in the Architecture section under either shape, with better placement under DR-G), and the "XL scope" estimate was inflated (realistic shape is M-L). The honest cut motivation was sunk cost on #195 plus familiarity bias toward "discovery produces tickets" — neither survives the user's stated criterion of best-long-term-option-even-at-upfront-cost. The user's reframe of discovery's posture ("high-level epic creation with rough ideas of what parts need to come together") describes DR-G, not DR-1. Accordingly: #195 is retired with a superseded-by pointer; #196 is created framed as DR-G; the empirical re-walk evidence transfers (it validated the per-piece ticket-body shape, which is what promote-piece will produce). [NOTE: this revision was itself reverted — see note 4 below.]

> **Revision note 4 (DR-G reverted; back to DR-1 with 9 carryovers)**: After committing #196 (DR-G) and beginning to design promote-piece UX in detail, an interview surfaced that the user wanted "discovery should create the epic AND the tickets in the backlog all in the same flow" — Reading B (DR-1) of the principal-architect reframe, not Reading A (DR-G) I had jumped to. Honest critical-thinking audit confirmed: DR-G was solving a hypothetical pain (stale-ticket accumulation in the backlog) that was NOT in the user's stated pain list of six items; DR-1's post-critical-review refined shape directly addresses all six pains. The DR-G detour was costly in tokens but produced 9 concrete strengthenings that carry over into DR-1's spec and #195's body: **(1) Architecture-section richness with worked examples elevated to spec-phase MUST deliverable, (2) Section-partitioned prescriptive-prose check runs on BOTH architecture-section at research-write AND ticket bodies at decompose-§5, (3) Lexical scanner ships as actual code not protocol prose (`bin/cortex-check-prescriptive-prose`), (4) "Why N pieces" reframed to falsification (attempt-merge) instead of justification, (5) Single permissive authoring paragraph for non-constructive topic shapes — diagnostic/policy/migration use the existing template with light guidance rather than branching sub-templates, (6) Concrete worked-example piece definitions in the authoring template, (7) Approval checkpoint adds a fourth option "Promote sub-topic to its own discovery", (8) Multi-epic and re-running-discovery edge cases addressed explicitly in #195's edges, (9) Drop-piece uses existing `cortex-update-item NNN status: closed` convention — no new workflow surface.** #196 closed with superseded_by: 195. #195 reopened with the 9 carryovers folded into its body. Feasibility table: Approach G reverts to cut; Approach A is the recommendation. The recursive-discovery option (carryover #7) is the most novel and valuable structural addition surfaced by the detour.
...
| **A. Coordinated discovery-side rewrite (DR-1, RECOMMENDED per revision note 4 — supersedes G after revert)** | M | Medium. Touches three files in one PR. Refined shape validated by pre-implementation re-walks against vertical-planning + repo-spring-cleaning; both produced architecturally-correct tickets at appropriate headcount with one gate-caught leak each. Strengthened by 9 carryovers from the DR-G detour — see revision note 4 and #195 body's "Edges from DR-G exploration" + "Spec-phase MUST deliverables" sections. Vague-mush risk on novel pieces remains the residual concern; refine-side DR-2 strengthening is the deferred backstop. | Spec-phase MUST deliverables: worked examples + anti-patterns in architecture-section authoring guidance; lexical scanner as actual code; concrete piece definition. |
| **G. Demote decompose-time ticket creation; pieces promoted on demand via new workflow** (CUT per revision note 4 — was briefly recommended in revision note 3) | M–L | Medium scope. Initially cut in revision note 2 for thin reasons; briefly re-adopted in revision note 3 under user pressure for "best long-term option even at upfront cost"; reverted in revision note 4 after the user clarified preference for "discovery should create the epic AND the tickets in the backlog all in the same flow" (Reading B / DR-1 of the principal-architect reframe, not Reading A / DR-G I had assumed). DR-G was solving a hypothetical pain (stale-ticket accumulation) not in the user's stated pain list. The 9 strengthenings surfaced by the DR-G exploration carry over to DR-1 — the detour wasn't wasted, just structurally unnecessary. | N/A — cut. |
