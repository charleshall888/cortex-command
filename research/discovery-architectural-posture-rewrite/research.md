...
|----------|--------|-------|---------------|
| **A. Coordinated discovery-side rewrite (DR-1 recommended, post-revision)** | M | Medium. Touches three files in one PR. Refined shape is a hypothesis informed by walk-through findings but not validated by re-walk. Vague-mush risk on novel pieces is partially-mitigated by Role/Integration/Edges template + piece-count justification; remainder is the spec-phase re-walk obligation. Mitigated by: spec-time re-walk against vertical-planning + one alternative corpus; uniform ticket template (no annotation pivot); section-partitioned gate that doesn't depend on agent self-classification; events.log instrumentation enabling future evidence-based escalation. | None — all changes in cortex-command's own skill files. |
| **B. Defer refine-side strengthening (DR-2 deferred)** | — | Low — the walk-through and revised DR-1 show discovery-side fix carries the load. If novel-piece vague-mush persists after A lands (the spec-phase re-walk will be the first signal), refine-side becomes the next ticket. Reversible deferral. | A's outcome. |
