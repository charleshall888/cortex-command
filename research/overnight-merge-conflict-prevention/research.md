...
- **Algorithm inversion required**: The current greedy phase in `group_into_batches()` treats tag overlap as a *grouping attractor* — items with high tag overlap land in the same batch. Area overlap must be treated as a *separation constraint* — items with overlapping areas must be forced into different rounds. When an item has both high tag overlap (current grouping pull) and area overlap (new separation push), area-overlap takes priority. This is not an additive field — it is a new constraint layer in the grouping logic. The tag-grouping objective and the area-separation objective can directly conflict; the implementation must define this resolution explicitly.
- **Net-new project limitation**: Area declarations are hardest to write precisely when they are most needed. On a fresh project with no established module boundaries, all 10 tickets from one discovery share tags and likely share the same new files — exactly the scenario where area declarations are most uncertain. On an established project with clear module boundaries, declarations are easy to write but conflicts are already less likely. For net-new projects, C+D (visibility and recovery improvements) are the primary near-term mitigation. The `areas:` approach pays off incrementally as the codebase develops structure and declaration confidence improves.
- **Trade-offs**: Declaration requires the AI writing the spec to predict file overlap. Even coarse area tags ("touches auth, users") reduce conflicts on established projects. Enforcement relies on humans/AI populating the field consistently — there is no fallback if it's absent or wrong.
...
- **Implementation risk**: The join between event log data and the state's feature dict is by feature name string. The event's `feature` field is written by `_apply_feature_result()` (`batch_runner.py` line 1356); the state dict key is set during feature initialization. If these write sites use different formats (slug vs. display title, with or without path prefix), the join silently produces nothing — the paused feature renders without conflict details, no error is raised, and the user sees the same degraded output as before. Implementation must explicitly verify that the feature name written to `merge_conflict_classified` events matches the key in `data.state.features` exactly before shipping.
- **Trade-offs**: None beyond verifying name normalization. Backlog 002 is adjacent; this may be folded into the same ticket or tracked separately.
