...
4. **Trigger and routing — does the autonomous path even exist today?**
   → **No autonomous path currently exists for `/cortex-interactive:lifecycle plan` invocation.** The overnight orchestrator's plan-gen sub-agents are a different mechanism and have not fired in production [research/overnight-plan-building/research.md:5-6, 89]. `/refine` does not invoke plan-phase [premise-unverified: not-searched in refine SKILL.md exhaustively, but codebase agent confirmed `NOT_FOUND(query="refine invokes plan", scope="plugins/cortex-interactive/skills/refine/")`]. The user's framing — "planning can happen overnight" — describes a future state, not a current one. Per CLAUDE.md OQ3 [CLAUDE.md:33-47], escalating from interactive-default to autonomous requires events.log evidence of the soft form failing. **No such evidence exists.** This produces a circularity that the artifact does not resolve internally: DR-1 calls for base-rate instrumentation, but instrumentation cannot accumulate signal until the autonomous path exists. **Discovery scope must therefore either (a) include path-building (e.g., `cortex overnight start --include-unrefined`) as a zeroth-zeroth epic preceding instrumentation, or (b) be re-aimed at async-notify-with-timeout (DR-6) for the existing interactive flow only — the choice depends on roadmap intent (load-bearing Open Question).**
...
### DR-1: Pause for roadmap conversation before any synthesis-design commitment (load-bearing gate)

- **Context**: The discovery's motivating premise — *"planning can happen overnight where the user may not even be able to respond"* — describes a future state, not a current one. `[research/overnight-plan-building/research.md:5-6, 89]` documents zero observed overnight invocations of plan-phase. The autonomous path for `/cortex-interactive:lifecycle plan` does not exist today (Q4). The synthesis design the user originally requested presupposes that path. The critical-review synthesis (events.log:critical_review, 2026-05-04) surfaced this circularity: any "defer until baseline" recommendation is unsatisfiable until the path exists; any synthesis-design commitment is premature without confirmation that overnight unattended plan-phase is a roadmap goal at all.
- **Options considered**:
  1. **Pause for roadmap conversation; produce backlog ticket for the conversation as discovery's primary output**
  2. Commit to building the autonomous path (`cortex overnight start --include-unrefined` or similar) as zeroth-zeroth epic preceding synthesis design
  3. Reduce scope to async-notify-with-timeout for existing interactive flow (DR-6 only); drop synthesis design entirely
  4. Synthesis-via-async-operator-decision: build the synthesis-presentation layer (DR-3 Option 4 + DR-4 + DR-5) but keep selection operator-driven via async push
- **Recommendation**: **Option 1**, per operator decision logged at events.log:critical_review (2026-05-04). The autonomous-path infrastructure is a substantial scope expansion that requires upstream confirmation; the synthesis-system design is meaningful only after that decision. Decomposition output: a single backlog ticket framing the roadmap conversation, with this research artifact as `discovery_source`. The remaining DRs (DR-2 through DR-7) remain valid conditional recommendations — they activate if/when the roadmap conversation resolves toward path-building or synthesis-via-async-operator.
- **Trade-offs**: Discovery does not produce an implementation epic. Acceptable: filing implementation tickets prematurely would commit scope the user has not chosen. The conversation is the right next step; the artifact preserves all conditional recommendations so post-conversation work can re-enter at the right point without re-running discovery.
