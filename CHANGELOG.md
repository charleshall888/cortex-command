...
- **Daytime autonomous pipeline** — the three daytime pipeline modules (`cortex_command/daytime/pipeline.py`, `cortex_command/daytime/session.py`, `cortex_command/daytime/dispatcher.py`), `cortex_command/daytime/readiness.py`, the three `[project.scripts]` console-scripts (`cortex-daytime-run`, `cortex-daytime-status`, `cortex-daytime-cancel`), seven test files (`tests/test_daytime_pipeline.py`, `tests/test_daytime_session.py`, `tests/test_daytime_dispatcher.py`, `tests/test_daytime_readiness.py`, `tests/test_daytime_integration.py`, `tests/test_daytime_cancel.py`, `tests/test_daytime_e2e.py`), dashboard daytime surfaces, the justfile `dispatch-parity` recipe, and associated `.gitignore` patterns are all removed.
