...
- **archive** — `cortex-update-item {{item}} --status complete|abandoned`. It updates in place, clears the item from others' `blocked-by`, closes parent epics whose children are all complete or abandoned, and regenerates the index. It closes an item in any status, so warn before closing a `backlog` or `in_progress` item.
