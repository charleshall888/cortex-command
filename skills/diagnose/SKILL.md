...
1. **Explicit feature argument** (e.g., `/cortex-interactive:diagnose my-feature`): write to
...
> (e.g., `/cortex-interactive:diagnose my-feature`) for reliable lifecycle-coupled artifact placement.
...
Invoking `/cortex-interactive:diagnose` when a lifecycle task fails is a structured pre-retry step — it does not
