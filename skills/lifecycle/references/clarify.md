...
Read `skills/lifecycle/references/clarify-critic.md` and follow its protocol. After the critic completes, the orchestrator writes the `clarify_critic` event to `lifecycle/{feature}/events.log` with the post-critic status.
...
If `update_item.py` fails, surface the error and ask the user to resolve it before proceeding. Do not silently skip write-backs.
