...
3. **Event logging** — lifecycle owns `cortex/lifecycle/{feature}/events.log`. After each phase, log a `phase_transition` — one row per boundary (clarify→research, research→specify): `cortex-lifecycle-event phase-transition --feature <name> --from <from> --to <to>`.
   The `specify→plan` boundary is NOT emitted here — the spec-approve verb (`cortex-lifecycle-spec-approve`, invoked by specify.md §4 under lifecycle-wrapped refine) owns that row and emits it when its transition flag is set; enumerating it here too would double-emit it.
