...
Stage `lifecycle/{feature}/` artifacts alongside source changes, then use `/cortex-interactive:commit` to create the commit. If `lifecycle.config.md` specifies `commit-artifacts: false`, exclude lifecycle artifacts from staging.

**If on a feature branch (not main/master):**
Push the branch and use `/cortex-interactive:pr` to create a pull request with a summary of the feature.
