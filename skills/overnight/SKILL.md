...
- **Parallel agent dispatch uses `Agent isolation: "worktree"`.** When launching features in parallel, always use the `Agent` tool with `isolation: "worktree"`. Do not call `git worktree add` manually — in sandboxed sessions this fails because the Seatbelt mandatory deny on `.mcp.json` (enforced by Anthropic's `sandbox-runtime`, below user-level `sandbox.filesystem.allowWrite`) blocks `git worktree add` from checking out `.mcp.json` into any path under the repo `.claude/` scope. Same-repo worktrees resolve to `$TMPDIR/cortex-worktrees/{feature}/` via `cortex-worktree-resolve`. A failed checkout still leaves an orphaned branch requiring `git branch -d <name>` cleanup before retrying.
