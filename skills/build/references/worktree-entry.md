...
Loaded from implement.md §1 on the worktree arm — the `resolved` state with `worktree-interactive`, or the `prompt` state when the picker selection is the worktree option (the verb returns `prompt` before the choice exists, so the load keys on the selection, not the verb state). §1 hands off one thing: the **entry mode** marker. Follow this to completion, then return to implement.md §2 — the session stays inside the worktree. That isolation is the payoff: same-file tasks dispatch concurrently instead of serializing, the trunk-mode cost this path avoids.

Route on the entry mode:

- **`selected`** — the user picked the worktree option. Run **Step A** first; a rejection returns to §1 without creating a worktree. On pass, continue to §1a.
- **`suppressed`** — `branch-mode: worktree-interactive` bypassed the picker. Skip Step A and go straight to §1a; its own overnight guard runs there.
...
Exit 0 = none active, proceed to §1a; 1 = overnight live, surface the wording and return to §1 without creating a worktree; 2 = stale runner, warn and continue to §1a.

### 1a. Worktree creation
...
3. **Already-in-worktree probe** (`selected`) — `cortex-worktree-precondition`. Exit 0 = not inside a worktree, proceed; exit 1 = already inside, skip op 4 and route to the fallback naming the detected worktree.
4. **Auto-enter** (`selected`, probe returned 0) — `EnterWorktree(path=<resolved-path>)` where the path is `cortex-worktree-resolve interactive-{slug}`'s output, never a hardcoded prefix. Sets session CWD to the worktree for all subsequent Bash calls and clears CWD-dependent caches. Any error (path not in `git worktree list`, schema rejection, "Must not already be in a worktree" race) → fallback.

**Fallback — `EnterWorktree skipped`.** On the `selected` path (op-3 probe non-zero, op-4 error, or the skill declines the tool): cd-shim handoff `cd $(cortex-worktree-resolve interactive-{slug})`, with a one-line diagnostic beginning `EnterWorktree skipped` naming the failure mode. Auto-enter affects only orchestrator-session Bash calls; §2's `Agent(isolation: "worktree")` dispatch and §2e merge-back are unaffected.
