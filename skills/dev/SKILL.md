...
Run the global shell command `generate-backlog-index` directly (it is a binary at `~/.local/bin/generate-backlog-index` — do NOT use `uv run`, `python`, or any interpreter prefix, and do NOT look for a project-local script). If it fails, warn that `index.json` may not be produced.
