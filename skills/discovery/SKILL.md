...
If brief generation exits non-zero, OR `brief.md` is missing after the command runs, OR `brief.md` fails decision-content validation, the gate falls back to displaying the dense `## Architecture` section (sub-sections `### Pieces` and `### How they connect`) and surfaces a warning naming the failure condition (`brief_generation_failed: <reason>`). No decompose work begins until the user answers. Four options:
