...
#: Re-exported from the side-effect-free ``cli_pin`` sibling so both
#: this module and the SessionStart-async sync hook share one
#: canonical declaration (Phase 1 of #245).
from cli_pin import CLI_PIN  # noqa: E402
...
#
# Install-path constants, helpers, and the ``_run_install_and_verify``
# runner have been factored out of this module into the stdlib-only
# sibling ``install_core`` (spec R9/R12, Task 7+8). They are imported
# below via **unqualified** names (``from install_core import …``) so
# they re-bind on this module's namespace and existing test patches
# (``monkeypatch.setattr(server, "_run_install_and_verify", spy)`` in
# ``tests/test_mcp_auto_update_real_install.py``) continue to intercept
# the install call without test-code changes (spec R12, R30, Non-
# Requirements: no qualified call style for the runner — the symbol
# must re-bind on this module's namespace).
#
# The plugin directory is added to ``sys.path`` ahead of the import so
# the sibling ``install_core.py`` resolves regardless of how server.py
# was loaded (PEP 723 single-file ``uv run``, in-process pytest module
# import, etc.). The existing in-function ``from install_guard import``
...
from install_core import (  # noqa: E402
    CortexInstallFailed,
    _INSTALL_SENTINEL_WINDOW_SECONDS,
...
def _ensure_cortex_installed() -> None:
    ...
    * If a recent ``install-failed.*`` sentinel exists (within
    ...
    if is_auto_install_disabled():
        # R19 notice-only path (spec R30 shared carve-out, now sourced
        # from ``install_core.is_auto_install_disabled``): the
        # missing-CLI error string already documents the user-facing
        # remediation. Returning silently lets the downstream
        # subprocess raise FileNotFoundError, which the existing
        # CortexCliMissing handlers translate into the canonical
        # user-facing error.
        return
...
#
# ``_NDJSON_ERROR_STAGES``, ``_last_error_log_path``, and
# ``_append_error_ndjson`` have been factored into the stdlib-only
# sibling ``install_core`` (spec R9/R12, Task 7+8) and are imported
# unqualified at the top of this module so all existing call sites in
# server.py resolve through this module's namespace.
# ---------------------------------------------------------------------------
