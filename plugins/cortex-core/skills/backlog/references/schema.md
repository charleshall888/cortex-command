...
| `lifecycle_phase` | string | no | Current lifecycle phase, or `null`. Value set: `research`, `specify`, `plan`, `implement`, `implement-rework`, `review`, `complete`, `escalated`. (`implement-rework` was added when lifecycle phase detection was unified around `cortex_command/common.py`.) |
