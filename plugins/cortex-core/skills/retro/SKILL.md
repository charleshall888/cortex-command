---
name: retro
description: Write a dated problem-only log for the current session. Accepts an optional context tag (e.g. `/cortex-core:retro lifecycle-work`). Use when user says "/cortex-core:retro", "write a retro", "session retrospective", or wants to log what went wrong in this session. Captures user corrections, mistakes made, things missed, and wrong approaches — each with its consequence. Does NOT capture what worked or accomplishments.
argument-hint: "[tag]"
inputs:
...
- **Non-empty after sanitization**: If tag is provided but becomes empty after sanitization (e.g. `"/cortex:retro ---"`), treat as if no tag was provided
...
- **Examples of invalid/rejected tags**: `Lifecycle_Work` (has uppercase and underscore) → sanitizes to `lifecycle-work` (OK); `/cortex:retro !!!` → sanitizes to empty (use default filename)

## Output Format Examples

### Example 1: Session with problems, no tag, no lessons

**Input**: `/cortex-core:retro` (invoked after a session with user corrections)
...
**Input**: `/cortex-core:retro feature-auth` (invoked after a clean session that included user-taught lessons)
...
5. Determine the filename:
   - If a tag argument was provided (any text after `/cortex-core:retro` on the command line), sanitize it:
     - Lowercase the entire string
     - Replace spaces with hyphens
     - Strip any character outside `[a-z0-9-]`
     - Collapse consecutive hyphens into a single hyphen
     - Trim leading and trailing hyphens
   - If the sanitized result is non-empty, use `$REPO_ROOT/retros/YYYY-MM-DD-HHmm-<tag>.md` (e.g. `/cortex-core:retro lifecycle-work` → `$REPO_ROOT/retros/2026-02-26-1430-lifecycle-work.md`)
...
8. Evolve nudge (skip entirely if `CLAUDE_AUTOMATED_SESSION` is set):
   ...
   - If count ≥ 10, append to the response: `"$count retros unprocessed — consider running /cortex-core:evolve."` (substituting the actual count). If count < 10, output nothing.
