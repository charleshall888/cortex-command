...
**a. Prepare shared context** — inject `{spec_path}` and `{research_path}` as absolute paths into the prompt template. Each agent reads the files itself; never share one agent's draft with another.
...
- **`verdict ∈ {A,B,C}` AND `confidence ∈ {high,medium}`** — present the chosen variant with the synthesizer's `rationale`; default is rubber-stamp (Enter), override by typing a different label. Write the chosen variant to `cortex/lifecycle/{feature}/plan.md`. Verdict `C` (tie) at high/medium is logically impossible — treat as malformed and fall to the table below.
...
`selection_rationale`/`selector_confidence` mirror the synthesizer's output, or a fallback string / `low` on the legacy-table path. `position_swap_check_result` is `agreed` at high/medium confidence, else `disagreed`. `disposition` is `rubber_stamp` (Enter), `override` (typed a different label), `deferred` (rejected all), or `auto_select` (overnight only, not emitted here). `operator_choice` is the chosen label or `null`.
