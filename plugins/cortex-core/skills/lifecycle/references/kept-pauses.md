...
|-------|------|---------------|----------|-----------|
| `ambiguous-backlog-pick` | question | — | `skills/lifecycle/SKILL.md` — Step 1: Read the served state (resolver `ambiguous-backlog` state) | Backlog match is ambiguous: present candidates for the operator to disambiguate, then re-run. |
| `empty-lifecycle-offer` | question | — | `skills/lifecycle/SKILL.md` — Step 1: Read the served state (resolver `empty` state) | No topic given and incomplete lifecycles exist: offer them for the operator to pick, then re-run. |
| `resume-feature-pick` | question | — | `skills/lifecycle/SKILL.md` — Situational references (listing incomplete features) | Multiple incomplete features and none specified: list them and ask which to resume. |
...
| `complete-merge-wait` | phase-exit-wait | — | `skills/lifecycle/references/complete-first-run.md` — Step 5 — Phase-exit pause | Hand off after opening the PR: operator merges on GitHub then re-invokes complete; manual re-invocation is the gate. |
| `complete-test-command-ask` | question | `test-command` | `skills/lifecycle/references/complete-first-run.md` — Step 1 — Tests | Config present but without test-command: ask whether tests exist; suppressed when test-command is set (just run it). |
| `complete-orphan-pr-pick` | question | — | `skills/lifecycle/references/complete.md` — Router verdict (`orphan_ambiguous`) | Multiple orphan PRs match interactive/<slug>: ask which PR to finalize before writing pr.json. |
...
| `implement-branch-pick` | config-conditional | `branch-mode` | `skills/lifecycle/references/implement.md` — 1. Pre-flight (`prompt` state picker) | Fallback branch picker on main; suppressed when branch-mode is set, the tree is clean, and no live worktree exists for the slug. |
| `plan-approval` | relayed-consent | — | `skills/lifecycle/references/plan.md` — 4. Approval (merged branch/dispatch surface) | Substantive plan approval merged with branch/dispatch selection; overnight relays the operator's pre-authorized consent. |
...
| `spec-interview-gapfill` | question | — | `skills/refine/references/specify.md` — 2. Interview | Structured spec interview: probe one question at a time via AskUserQuestion until ambiguities resolve. |
