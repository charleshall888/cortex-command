...
Determine the feature name from the invocation. Use lowercase-kebab-case for directory naming. When the lifecycle is linked to a backlog item, derive the directory name using the canonical `slugify()` from `cortex_command/common.py`: lowercase, convert underscores and slashes to spaces, strip remaining non-alphanumeric characters (except hyphens and spaces), collapse runs of spaces/hyphens to a single hyphen. Underscores become hyphens, not stripped (e.g., `session_panel` → `session-panel`, not `sessionpanel`). Leading/trailing hyphens are stripped. Example: backlog title "Build overnight runner and orchestrator round loop" → `lifecycle/build-overnight-runner-and-orchestrator-round-loop/`. This ensures the overnight selection module can discover the lifecycle artifacts automatically.
