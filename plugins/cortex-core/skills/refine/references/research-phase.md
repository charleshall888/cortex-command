...
If `cortex/lifecycle/{lifecycle-slug}/research.md` exists, apply the Research Sufficiency Criteria (`clarify.md` §6) against Clarify's intent and scope. **Path guard**: only a file at that exact path counts — a backlog item's `discovery_source`/`research` field is background only (per the Refine Starting-Point Rules in `refine-delegation.md`). Missing → run Research Execution.
