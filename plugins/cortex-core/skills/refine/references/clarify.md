...
2. **Complexity** — `simple` (1–3 files, mechanical, an existing pattern followed exactly, no behavioral effect on callers) or `complex` (4+ files, cross-cutting effects, design trade-offs, or new patterns). "Touches shared infrastructure" is not by itself complex — in a harness repo that matches everything; ask whether the change *reshapes* it or follows it. **When in doubt prefer `simple`**: `cortex-complexity-escalator` ratchets up when research surfaces open questions, so an underestimate self-corrects while an overestimate never does.
