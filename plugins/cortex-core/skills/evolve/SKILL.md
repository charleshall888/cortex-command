---
name: evolve
description: Identify recurring problems across retro logs and route each trend to the appropriate skill for investigation or resolution. Use when user says "/cortex-core:evolve", "evolve", "find trends in retros", "process retros", "what keeps going wrong", or wants to analyze session retrospectives to surface patterns and route improvements to the right workflow.
...
- `/cortex-core:evolve` — process up to the last 5 unprocessed retros
- `/cortex-core:evolve N` — process up to the last N unprocessed retros (e.g., `/cortex-core:evolve 10`)
...
> "No retros found — run /cortex-core:retro to start the feedback loop"
...
`last_processed` is the filename of the newest retro from the previous `/cortex-core:evolve` run. Only retro files with filenames **lexicographically greater** than `last_processed` are unprocessed. If no state file exists, all retros are unprocessed.
...
2. **Validate the cortex-command marker**: check that `$REPO_ROOT/skills/evolve/SKILL.md` exists. If it does not, emit the following two-line error to stderr and exit non-zero:
   - Line 1: `/cortex-core:evolve must be invoked from inside a cortex-command checkout (git rev-parse --show-toplevel resolved to "<resolved-path>" but skills/evolve/SKILL.md not found there)`
...
| Situation | Route |
|-----------|-------|
| Unknown or complex root cause | `/cortex-core:discovery <topic>` |
| Understood, non-trivial fix | `/cortex-core:lifecycle <feature>` |
| Simple, scoped improvement | `/cortex-core:backlog add` |
| Immediate config or memory update | `/claude-md-improver` or direct MEMORY.md/CLAUDE.md edit |

Before proposing `/cortex-core:backlog add`, scan `backlog/*.md` for an open item with matching title keywords. If one exists, note it rather than creating a duplicate.
...
|---|-----------------|-------------|----------------|
| 1 | <description>   | 2 (retros: YYYY-MM-DD-HHmm, ...) | /cortex-core:discovery <topic> |
| 2 | ...             | 3 | /cortex-core:lifecycle <feature> |
...
- `/cortex-core:discovery <topic>` — invoke the discovery skill
- `/cortex-core:lifecycle <feature>` — invoke the lifecycle skill
- `/cortex-core:backlog add` — create a backlog item following the repo's backlog format (`backlog/NNN-title.md` with YAML frontmatter)
