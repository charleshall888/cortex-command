...
Runs only when `/cortex-core:discovery` is invoked with no topic argument. Does not write any files or state — it is read-only and produces only a `{{topic}}` for the normal discovery flow.
...
- If it does not exist: output "No requirements docs found — cannot auto-scan for gaps. Provide a topic to start discovery: `/cortex-core:discovery <topic>`." and exit.
...
If all candidates are covered: output a coverage summary (e.g., "All requirements areas have active backlog or lifecycle coverage.") and exit cleanly. Do not prompt for a topic — the user can call `/cortex-core:discovery <topic>` manually.
