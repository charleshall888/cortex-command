---
name: fresh
description: Capture the current session state as a resume prompt you can paste into a fresh context window. Use when user says "/cortex:fresh", "fresh context", "context is full", "start fresh", or wants to continue work in a new window. Reads the conversation, identifies ephemeral context not captured in files, and outputs a ready-to-paste prompt.
...
0. **Run /cortex:retro if this is a human-initiated session.** Check whether this is an automated session:

   ```bash
   [ "${CLAUDE_AUTOMATED_SESSION:-0}" = "1" ]
   ```

   - If the variable is **not set** (exit code 1 — human session): run `/cortex:retro` now, before proceeding, while full session context is still available.
...
1. **Read the conversation.** Identify:
   ...
   - If a lifecycle feature was being worked on **in this conversation**: feature name, current phase, last completed task, resume command (e.g. `/cortex:lifecycle feature-name`)
...
2. **Generate an adaptive resume prompt.**
   ...
   - Include resume commands where relevant (e.g. `/cortex:lifecycle feature-name`, `/pipeline resume`)
