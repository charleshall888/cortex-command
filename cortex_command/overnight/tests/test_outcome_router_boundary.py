"""Enforces that outcome_router.py does not import from batch_runner —
...
class TestOutcomeRouterImportBoundary(unittest.TestCase):
    def test_no_forbidden_imports(self) -> None:
        source = _OUTCOME_ROUTER_PATH.read_text(encoding="utf-8")
        ...
        if violations:
            self.fail(
                "outcome_router.py contains forbidden runtime imports:\n"
                + "\n".join(violations)
            )
