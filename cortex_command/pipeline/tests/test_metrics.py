...
def test_v2_tolerance_downstream_filter_unaffected(tmp_path):
    """A mixed events.log containing one HISTORICAL v2 ``plan_comparison``
    row and one ``feature_complete`` event: callers that filter the parsed
    list by ``event == "feature_complete"`` get exactly the
    ``feature_complete`` event back.

    ``plan_comparison`` emitters were deleted (#391 overnight, #398
    interactive — zero production readers), but rows survive in archived
    per-feature logs. Adjacent readers (``cortex_command/dashboard/data.py``,
    ``claude/statusline.sh``, ``bin/cortex-archive-sample-select``,
    ``hooks/cortex-scan-lifecycle.sh``) all key on the ``event`` field name;
    this test guards that a foreign historical row neither breaks
    ``parse_events`` nor contaminates name-keyed filters.
    """
    import json as _json
