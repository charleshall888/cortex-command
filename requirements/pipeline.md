...
- **Acceptance criteria**:
  ...
  - The morning report is written to two paths: `lifecycle/sessions/{session_id}/morning-report.md` (gitignored per-session archive) and `lifecycle/morning-report.md` (tracked latest copy); the latter is committed to local `main` directly by the runner process (not a runner-spawned child). Phase 0 does not block this commit because `CORTEX_RUNNER_CHILD` is unset in the runner process — only spawned child commits get gated.
