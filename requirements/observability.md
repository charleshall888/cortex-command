...
- **Install-mutation**: only `cortex_command.cli._dispatch_upgrade()` — runs `uv tool install ... --force` against the tag-pinned git URL. The guard is invoked as the first statement of that handler.
...
- **Install-mutation orchestrator (first install hook)**: `plugins/cortex-overnight-integration/server.py:_ensure_cortex_installed()` runs before each MCP tool handler delegates to a `cortex` subprocess; on cortex-absent it shells out to `uv tool install --reinstall git+<url>@<tag>` and logs failures to `${XDG_STATE_HOME}/cortex-command/last-error.log` with `stage: "first_install"` (per spec R7f). The spawned install does not currently route through `_dispatch_upgrade`, so no in-flight guard call fires at this site — acceptable because first-install runs at most once per host before any other install-mutation entry point can race.
