...
- Commit with `git commit --only -m "..." -- <the same paths you staged>` — concurrent sessions share one git index, so a bare `git commit` sweeps whatever a sibling session staged. Stage specific paths, never `-A`. A shared hook validates messages (imperative mood, capitalized, no trailing period, max 72-char subject); if it rejects a commit, fix the message rather than bypassing it via `git commit -F` or an editor, which the hook cannot see.
...
- Editing `skills/`, `hooks/`, `claude/hooks/`, `bin/cortex-*`, or `cortex_command/common.py` carries no lifecycle gate — edit them directly. Route through `/cortex-core:dev` when the change is large or unclear enough to want research and a spec, not because of where the file lives: a gate that fires on a path fires hardest on the one-line doc fix, which is the change least in need of a lifecycle. Edit canonical sources only; the `plugins/cortex-core/{skills,hooks,bin}/` mirrors are rebuilt from your staged blobs by the pre-commit hook and folded into the commit, so never stage them by hand — and expect the commit to contain mirror paths you did not name.
